// 试玩辅助工具：把 observe/act 输出的完整 JSON 压成决策摘要。
// 只整理观察，不选择或提交游戏行动。
...
const h = g.life?.heir;
line('本人', `年龄${p?.ageYears} 健康${p?.health} 精力${p?.energy}/${p?.maxEnergy} 天赋:${p?.talent?.name}(${p?.talent?.effect})`);
line('后辈', `年龄${h?.ageYears} 健康${h?.health} 精力${h?.energy}/${h?.maxEnergy} 天赋:${h?.talent?.name}(${h?.talent?.effect})`);
