...
export function validateRuleset(value: unknown): Ruleset {
  ...
  if(!isRecord(value.farm)||Object.keys(value.farm).length!==Object.keys(FARM_BOUNDS).length||Object.entries(FARM_BOUNDS).some(([key,[min,max]])=>!Number.isInteger((value.farm as Record<string,unknown>)[key])||Number((value.farm as Record<string,unknown>)[key])<min||Number((value.farm as Record<string,unknown>)[key])>max))throw new Error('缺少有效的地块与邻居规则，请新开游戏；旧档不迁移');
