...
export interface FarmPlot {land?:PlotLand;project?:{kind:FarmProjectKind;done:number;total:number};improvement?:'canal'|'shelter'|'drain'|'yard'|'cellar'|'pit'|'shed'|'retting';pit?:{readyDay:number};id:string;x:number;y:number;kind:'unknown'|'wild'|'field'|'tree'|'rock'|'brush'|'story'|'water';field?:Field;discovery?:{id:FarmDiscovery;resolved:boolean;outcome:string};fertility?:number;}
