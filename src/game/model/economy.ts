...
export type FarmProjectKind = 'canal'|'restore'|'timber'|'clearwood'|'shelter';
export interface FarmPlot {project?:{kind:FarmProjectKind;done:number;total:number};improvement?:'canal'|'shelter';id:string;x:number;y:number;kind:'unknown'|'wild'|'field'|'tree'|'rock'|'brush'|'story';field?:Field;discovery?:{id:FarmDiscovery;resolved:boolean;outcome:string};fertility?:number;}

export interface FarmState {explorationVersion:3;rareSeeds:number;rules:FarmRules;plots:Record<string,FarmPlot>;discovered:Crop[];explored:number;neighbor:{personId:string;goods:Record<string,number>;field:Field;talked:number;traded:number;helped:number;busy:boolean};}
