...
export interface ActionCost { ap: number; time?: number; energy?: number; money: number; food: number; materials?: Partial<Record<Material, number>> }
...
export function parseActionId(id: string): GameAction {
  ...
  if(type==='economy'&&parts.length===3&&['cook','farmrare','farmstory','farmplot','farmexplore','farmreclaim','farmfertilize','neighbor','bond','sectswitch','sectseek','sectadmit','sectpractice','sectteach','sectimprove','sectdraw','crisis','tap','dungeonstart','dungeonwork','erasettle','publicmill','foodpolicy','foodbudget','foodreserve','inspect','sysbuild','syscommission','sysassign','sysrun','sysremove','branchlearn','branchteach','brancharchive','channel','rest','care','retire','expeditionstart','expeditionpause','expeditionship','utility','energize','nutrient','reclaim','towerstart','towerpause','towerdelivery','workshopbuild','workshoppause','workshopexpand','workshoptransport','workshophousehold','workshopupstream','resumeplans','foodplan','farmplan','productionplan','supplyplan','salesplan','careplan','charter','projectstart','projectpause','mineplan','steamplan','cartadd','cartremove','checkout','clearcart','repair','tuition','paidtrain','study','research','archive','teach','publish','gather','work','buy','sell','sellfood','buyfood','farm','farmcycle','fertilize','build','process','finish','hire','assign','pause','train','company','consult','end'].includes(nodeId)&&/^[a-zA-Z0-9-]+$/.test(practiceId))return {type,operation:nodeId,target:practiceId};
