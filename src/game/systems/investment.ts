...
export function technologyVictory(s:GameState,rules:Ruleset){
  ...
  if(s.era){const mastered=branchNodesFor(s).filter(n=>nodeInEra(s,n.id)&&Object.values(s.economy!.branches!.learned).some(ids=>ids.includes(n.id))).map(n=>n.id);return {mastered,total:branchNodesFor(s).length,required:0,achievements:s.era.dungeon.complete?['final-family-trial']:[],requiredAchievements:1,percent:Math.floor(s.era.dungeon.progress/s.era.rules.dungeonTarget*100),targetPercent:100,achieved:s.era.closed&&s.era.dungeon.complete,won:s.status==='complete'&&s.era.dungeon.complete};}
