...
export function modernActions(s:GameState):ActionDefinition[]{
 ...
 out.push(defineAction(s,'economy:energize:now','执行本季供能与服务','能源',{ap:1},s.economy!.operations?.paused?['先接续家业']:[],s.economy?.industry?'本人执行水力供能，消耗操作时间精力及实际公共水和耐用；季末不再免费发电。':'按水电、燃料、光伏、储能、通信、控制、冷库、温室、传感顺序运行；发电每设备每季最多一次。季末也自动尝试。',(d,ev)=>{generateModern(d,ev);serveModern(d,ev);}));
