...
export function actionDefinitions(state: GameState, rules: Ruleset): ActionDefinition[] {
  ...
  if (state.status === 'handover') {
    ...
    return [defineAction(state, 'handover', state.sect?'交接给自己的成年弟子':eraEnd?'结束本时代，开始新人物':'交接给后辈', '传承', { ap: 0 }, [], state.sect?'自己的成年弟子接班，同门不参与交接；门派道法与资产保留，个人修为和所学不复制。':eraEnd?'结清本时代实际回报，下一时代从无亲属关系的新随机角色开始；个人知识从基础开始。':'保留后辈真实学习状态、家学、资产和未完项目；前代个人能力不会复制。', (draft, events) => handover(draft, rules, events))];
  }
