...
export function actionDefinitions(state: GameState, rules: Ruleset): ActionDefinition[] {
  ...
  if(state.economy)return [...socialFoodActions(state),...industryActions(state),...lifeActions(state),...branchActions(state,rules),...economyActions(state,rules)].filter(a=>!state.economy?.branches||!branchActionNeeds(state,a.offer.id).some(reason=>reason.includes('尚未')||reason.includes('当前仅')));
