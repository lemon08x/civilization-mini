...
export function eraActions(s:GameState):ActionDefinition[]{
 ...
 out.push(defineAction(s,'economy:dungeonstart:family','进入最终副本：跨代家业试炼','最终副本',{ap:0},e.dungeon.started?['已经进入副本']:[],'仅现代社会开放。不是离开现代的条件；完成副本并结算后才算旅程胜利。',(d,ev)=>{d.era!.dungeon.started=true;eraEvent(d,ev,'dungeon-start','最终副本开始：为跨代家业工程提供真实供给或专业劳动');}));
 const options=[
 ...
 for(const o of options)out.push(defineAction(s,'economy:dungeonwork:'+o.id,o.name,'最终副本',{time:o.time,energy:o.energy,money:o.money},[...(!e.dungeon.started?['先进入最终副本']:[]),...(e.dungeon.complete?['副本已完成，无需继续交付']:[]),...o.needs],`提供${o.progress}份工程进度，需达到${e.rules.dungeonTarget}份。交付的商品真实移出家庭，专业施工由外部人员承担，数学验算是本人的工程劳动。`,(d,ev)=>{
  ...
  const task=d.era!.dungeon;task.progress=Math.min(d.era!.rules.dungeonTarget,task.progress+o.progress);task.complete=task.progress>=d.era!.rules.dungeonTarget;
