...
export function operationsActions(s:GameState,r:Ruleset):ActionDefinition[]{
 ...
 for(const active of [true,false]){
  ...
  add('foodplan',t,active?'签订长期供粮协议':'暂停供粮协议',{},[...(active&&org<1?['需生产组织1阶或家族记录']:[]),...(o.food===active?['已经是此安排']:[])],`季末按需购入至${foodTarget(s,r)}份可食储备，按普通粮价付费、不花行动；每季先从市场预留供应，缺钱才停供。`,(d,ev)=>{const ops=d.economy!.operations!;ops.food=active;if(active)reserveFood(d,r,ev);else{d.production!.market.food+=ops.foodReserved;ops.foodReserved=0;}opEvent(ev,'plan','food',active?'长期供粮已签订':'供粮已暂停');});
