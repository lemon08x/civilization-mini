...
import { accessible, has, practice, study, studyRequired, teach, techById } from '../systems/learning.js';
...
export function educationActions(state: GameState, rules: Ruleset): ActionDefinition[] {
  ...
  for (const tech of rules.technologies) {
    const progress = person.learning[tech.id] ?? 0, required = studyRequired(rules, person, tech);
    ...
    for (const tag of tech.practices) {
      ...
      actions.push(defineAction(state, `practice:${tech.id}:${tag}`, `实践：${rules.practiceNames[tag]}`, '实践', { money: tag === 'selection' ? 0 : p.trainingCost, food: tag === 'selection' ? 1 : 0 }, [
    ...
    const heirMissing = tech.prerequisites.filter(id => !has(child, id));
    const heirNeedsStudy = (child.learning[tech.id] ?? 0) < studyRequired(rules, child, tech);
    const heirPractice = tech.practices.find(tag => !child.practices.includes(tag));
    actions.push(defineAction(state, `teach:${tech.id}`, `${heirNeedsStudy ? '教导' : '带后辈练习'}：${tech.name}`, '传承', {}, [
      ...(!has(person, tech.id) ? ['自己尚未掌握'] : []), ...(heirMissing.length ? ['后辈缺少前置基础'] : []), ...(has(child, tech.id) ? ['后辈已经掌握'] : []),
