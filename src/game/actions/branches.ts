...
import { gainPractice } from '../systems/life.js';
...
export function branchActions(s:GameState,r:Ruleset):ActionDefinition[]{
  ...
  for(const node of branchNodesFor(s)){
    ...
    out.push(defineAction(s,`economy:branchlearn:${node.id}`,trial?'试种并掌握：'+node.name+(trialCrop&&trialPlot?`（${trialPlot} · ${CROPS[trialCrop].name}）`:''):'学习：'+node.name,'分支',{},[
      ...unavailable,...trialNeeds,...branchNeeds(s,node.parents),...(branchHas(s,node.id)?['已经掌握']:[]),...missingGoods(s,sample),
    ],`${trial?`实践学习：${trialCrop&&trialPlot?`在 ${trialPlot} 播种${CROPS[trialCrop].name}，消耗1份${CROPS[trialCrop].name}种子`:'发现豆种或麻种并准备一块空田'}；播种后掌握本课，作物正常生长与结算。也可向同门请教本课。`:''}${trial?'试种计入本次行动，无需再播种。':s.economy!.industry?'学科只检查前置知识；部分课程要社会发展到相应阶段才开放。学习不消耗样品。':node.benefit+'。'}${s.economy!.industry?(archived?'门派记录减少学习时间。':''):archived?'按门派记录学习，无需重复消耗实验样品':'通过地方入门指导与实物练习学习；样品：'+(Object.entries(sample).map(([k,n])=>`${k}×${n}`).join('、')||'无')}。${ancestorKnows(s,node.id)?'前代已学：仍须先学前置，本节点学习成本大幅降低。':s.life?.renewal?'本代首次探索；学会后师承弟子学习成本大幅降低。':''}${s.life?.consultPending===node.id?'长辈已指点：本次学习时间减少。':''}天赋影响时间精力报价。${s.electric&&ELECTRIC_EPIGRAPHS[node.id]?ELECTRIC_EPIGRAPHS[node.id]:''}`,(d,ev)=>{
