import {farmWork,plotField,farmWaterSupport} from '../systems/agriculture.js';
...
import type {Crop} from '../model/economy.js';
...
const TRIAL_LESSONS:Record<string,{crops:Crop[];paddy?:boolean;seedNeed:string;plotNeed:string;hint:string}>={
 ...
 A12:{crops:['rice'],paddy:true,seedNeed:'先持有至少1份稻种；建成渠后可向同门换种',plotNeed:'水田试种需要一块相邻渠格的空田（水分支持≥2）',hint:'备妥稻种并准备一块相邻渠格的空田'},
...
export function branchActions(s:GameState,r:Ruleset):ActionDefinition[]{
  ...
  for(const node of branchNodesFor(s)){
    ...
    const trialPlot=spec?Object.values(s.economy!.farm!.plots).find(p=>p.kind==='field'&&!plotField(s,p.id)?.crop&&(!spec.paddy||farmWaterSupport(s,p)>=2))?.id:undefined;
