...
export function branchActions(s:GameState,r:Ruleset):ActionDefinition[]{
  ...
  for(const node of BRANCH_NODES){
    ...
    ],`${s.economy!.industry?'学科只检查前置知识，地方指导始终可用；学习不消耗样品。':node.benefit+'。'}${s.economy!.industry?(archived?'家学减少学习时间。':''):archived?'按家学学习，无需重复消耗实验样品':'通过地方入门指导与实物练习学习；样品：'+(Object.entries(sample).map(([k,n])=>`${k}×${n}`).join('、')||'无')}。${ancestorKnows(s,node.id)?'前代已学：仍须先学前置，本节点学习成本大幅降低。':s.life?.renewal?'本代首次探索；学会后直系后代学习成本大幅降低。':''}天赋影响时间精力报价。`,(d,ev)=>{
