...
export function marketPage(g:SessionObservation['game'],category:string,filter:string,button:(id:string)=>string):string{
 ...
 const quote=v.quote;
 const cart=`<h3>采购清单</h3><p>编辑清单免费、不占库存；结账时间与精力见按钮。本季剩余运输 ${v.transport}，本单占用 ${quote.weight}。</p>${quote.lines.length?`<ul>${quote.lines.map(({item,quantity})=>`<li>${esc(item.name)} × ${quantity} · ${item.price*quantity}钱 · ${item.local?'现货，即刻到账':'下一季交付'} ${small('economy:cartremove:'+item.id,'减一份')}</li>`).join('')}</ul>`:'<p>还没有选择商品。</p>'}<p><strong>合计 ${quote.total}钱</strong> · 购买后剩余 ${quote.remainingMoney}钱${quote.remainingMoney<0?'（资金不足）':''}</p><div class="inspector-action">${button('economy:checkout:cart')}</div>${Object.keys(e.shop!.cart).length?small('economy:clearcart:all','清空清单'):''}
   <h3>待交付</h3>${v.orders.length?`<ul>${v.orders.map(o=>`<li>${esc(o.name)} × ${o.amount} · 第${o.due}总季开始完成；${o.kind==='repair'?'设备本季停用':o.kind==='training'?'员工本季停工、不扣工资':'已付款，无需再花行动'}</li>`).join('')}</ul>`:'<p>没有待交付订单。跨季到货不会画成即刻入账。</p>'}<p>资产：${v.assets.map(x=>x==='granary'?'家庭粮仓':'家学书室').join('、')||'暂无'}；家庭教材 ${v.books.length} 册，均可跨代使用。</p>${v.books.length?`<details><summary>查看已有教材</summary>${v.books.map(id=>`<p>${esc(TOPICS.find(t=>t.id===id)?.name)}</p>`).join('')}</details>`:''}`;
 ...
 return `<p>钱可以换取物资、已安装设备、学习服务与家庭资产。现货即刻使用；订货下一季开始交付。工资、食品、运输和在途库存只显示当前观察支持的事实。</p>
