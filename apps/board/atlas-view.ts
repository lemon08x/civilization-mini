...
type Entry={id:string;name:string;category:string;planned:boolean;source:string;use:string;art?:string;amount?:number;lore?:SeedLore};
const categories=['种子','烹饪菜肴','食材与材料','工具与加工品'];
let category='种子',status='全部',query='',selected='',seedStage='全部',seedSeason='全年',loreTab='history';
// Future entries describe design directions only; they never create goods or recipes.
const plans: [string,string,string,string][]=[
 ['小米粥','烹饪菜肴','小米 + 水，以柴火慢煮','农舍主食方向；具体恢复效果待定'],
 ['菘菜豆羹','烹饪菜肴','菘菜 + 大豆，以陶锅炖煮','让蔬菜与豆类进入一餐；配方待定'],
 ['蒸饼','烹饪菜肴','面粉和面，用蒸具熟制','连接磨粉与烹饪；蒸具规则待定'],
 ['葱油拌面','烹饪菜肴','面粉 + 植物油 + 葱','连接制面与榨油；配料规则待定'],
 ['萝卜炖豆','烹饪菜肴','萝卜 + 大豆，慢火炖煮','根菜家常饭方向；收益待定'],
 ['节令米糕','烹饪菜肴','黏性米粉蒸制','节日烹饪方向；不代表现有节日奖励'],
 ['豆腐','食材与材料','大豆浸泡、磨浆、凝固','豆类精加工 → 炖菜与交易'],
 ['干菜','食材与材料','蔬菜晾晒','跨季保存方向；损耗与保质期待定'],
 ['腌菜','食材与材料','蔬菜 + 盐，入缸腌渍','连接制盐、陶器与保存工艺'],
 ['稻米','食材与材料','稻谷脱壳','主食与米制点心的原料'],
 ['蚕丝','食材与材料','桑叶养蚕、缫丝','纺织原料；养蚕机制尚未开放'],
...
function entries(g:SessionObservation['game']):Entry[]{
 ...
 const meals=COOKING.map((r):Entry=>({id:'meal-'+r.id,name:r.name,category:'烹饪菜肴',planned:false,source:Object.entries(r.inputs).map(([id,n])=>`${e.goodsCatalog[id]?.name??id} ${n}`).join(' + '),use:`农舍现做现吃；餐食调养 ${r.food} 天，不存为仓库物品，效果期间不能叠加。实际时间、精力以行动报价为准。`,art:`farm-home-${r.id}.webp`}));
 ...
 const goods=Object.entries(e.goodsCatalog).filter(([id])=>!id.startsWith('seed')).map(([id,c]):Entry=>({id,name:c.name,category:uses[id]||/矿|原料|铁料|铜料|铝料/.test(c.name)?'食材与材料':'工具与加工品',planned:false,source:sources(id)?'加工来源：'+sources(id)+'。需满足对应知识与设备条件。':'随阶段与条件开放，查阅主场景的探索或买卖',use:(uses[id]??'')+(destinations(id)?' 可用于：'+destinations(id)+'。':' 可在开放的买卖中查看交易条件。'),art:`item-${id}-ui.webp`,amount:e.goods[id]??0}));
 ...
 return [...seeds,...meals,...goods,...tools,...plans.map(([name,category,source,use],i):Entry=>({id:'plan-'+i,name,category,source,use,planned:true}))];
}
export function atlasPage(g:SessionObservation['game']):string{
 const all=entries(g);
 return `<section class="item-atlas"><header class="atlas-intro"><div><span class="eyebrow">万物小志</span><h3>从一粒种子，到一桌饭菜</h3><p>已实装 ${all.filter(x=>!x.planned).length} 项 · 规划中 ${all.filter(x=>x.planned).length} 项。已实装不代表当前阶段可得；规划内容暂不参与游戏结算。</p></div><img src="/illustrations/item-book-ui.webp" alt="" width="90" height="90"></header>${seedReference()}<div class="atlas-toolbar"><div role="group" aria-label="图鉴分类">${['全部',...categories].map(c=>`<button type="button" data-atlas-category="${c}" aria-pressed="${category===c}">${c} <small>${c==='全部'?all.length:all.filter(x=>x.category===c).length}</small></button>`).join('')}</div><label>内容 <select id="atlas-status">${['全部','已实装','规划中'].map(s=>`<option ${s===status?'selected':''}>${s}</option>`).join('')}</select></label><label class="atlas-search">搜索 <input id="atlas-search" type="search" placeholder="名称、来源或用途" value="${esc(query)}"></label></div>${seedFilters()}<div class="atlas-workbench"><div><p id="atlas-count" class="subtle" aria-live="polite"></p><div class="atlas-grid">${all.map(x=>`<button type="button" class="atlas-card" data-atlas-id="${x.id}" data-category="${x.category}" data-planned="${x.planned}" data-seed-stage="${x.lore?.stage??''}" data-seed-seasons="${x.lore?.seasons.join('')??''}" data-search="${esc([x.name,x.source,x.use,x.lore?.history,x.lore?.science,x.lore?.story.text,x.lore?.region,x.lore?.tags.join(' ')].join(' '))}"><span class="atlas-picture">${x.art?`<img src="/illustrations/${x.art}" onerror="this.hidden=true" alt="" loading="lazy">`:`<span class="atlas-specimen" aria-hidden="true">${x.name.slice(0,1)}</span>`}</span><span><small>${x.category} · ${x.planned?'规划中':'已实装'}</small><strong>${esc(x.name)}</strong><em>${x.amount!==undefined?'库存 '+x.amount:x.planned?'未来设想':'查阅用途'}</em>${x.lore?`<span class="seed-card-seasons" title="${esc(x.lore.region)}">${['春','夏','秋','冬'].map(t=>`<i class="${x.lore!.seasons.includes(t as '春'|'夏'|'秋'|'冬')?'sow-season':''}">${t}</i>`).join('')}<small>${x.lore.firstBatch?'首批优先':['农场','工坊','贸易','现代'][x.lore.stage]}</small></span>`:''}</span></button>`).join('')}</div><p id="atlas-empty" hidden>没有找到相关条目，试试其他分类或关键词。</p></div><aside class="atlas-detail" aria-label="物品详情">${all.map(x=>`<article data-atlas-detail="${x.id}" hidden><span class="atlas-badge">${x.planned?'规划中 · 尚未实装':'已实装 · 开放条件以游戏为准'}</span><h3>${esc(x.name)}</h3><p class="subtle">${x.category}${x.amount!==undefined?' · 当前库存 '+x.amount:''}</p><h4>${x.planned?'设想来源':'如何获得'}</h4><p>${esc(x.source)}</p><h4>${x.planned?'发展方向':'有什么用'}</h4><p>${esc(x.use)}</p>${x.lore?seedDetail(x.lore):''}${x.planned?'<p class="atlas-note">这是未来内容储备，尚无可执行配方、效果数值或获取入口。</p>':''}</article>`).join('')}</aside></div></section>`;
}
export function bindAtlas():void{
 const cards=Array.from(document.querySelectorAll<HTMLButtonElement>('[data-atlas-id]'));
 const refresh=()=>{
  let count=0;
  for(const c of cards){const d=c.dataset;c.hidden=!(category==='全部'||d.category===category)||!(status==='全部'||(d.planned==='true')===(status==='规划中'))||(category==='种子'&&((seedStage!=='全部'&&d.seedStage!==seedStage)||(seedSeason!=='全年'&&!d.seedSeasons?.includes(seedSeason))))||!d.search!.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase());if(!c.hidden)count++;}
  ...
  document.getElementById('seed-filters')!.hidden=category!=='种子';
...
function seedFilters():string {
 return `<section id="seed-season-guide" class="seed-season-guide" ${category!=='种子'?'hidden':''}><header><strong>四时种植方向</strong><span>江淮地区作为首批设计参考 · 农时知识尚未作为播种限制实装</span></header><div>${[['春','粟豆瓜麻 · 稻作准备'],['夏','小豆大豆 · 适地接茬'],['秋','大小麦 · 葵芜菁芥菜'],['冬','暖区耐寒菜 · 麦类越冬']].map(([season,text])=>`<article><b>${season}</b><span>${text}</span></article>`).join('')}</div><p>冬播仅适用于温暖地区及相应品种；越冬不等于冬季快速生长。寒冷地区以整地、修渠、留种为主。具体月份须结合当地霜期与品种，不能直接把公历月份套入农历。</p></section><div id="seed-filters" class="seed-filters" ${category!=='种子'?'hidden':''}><label>发展阶段 <select id="seed-stage"><option value="全部" ${seedStage==='全部'?'selected':''}>全部阶段</option>${CHINESE_ERA_REFERENCE.map((e,i)=>`<option value="${i}" ${seedStage===String(i)?'selected':''}>${e.name}</option>`).join('')}</select></label><label>播种季节 <select id="seed-season">${['全年','春','夏','秋','冬'].map(t=>`<option ${seedSeason===t?'selected':''}>${t}</option>`).join('')}</select></label><span>季节亮点表示参考窗口，须同时阅读地域条件。种薯、薯苗也归入种植材料。</span></div>`;
}
function seedDetail(l:SeedLore):string {
 return `<section class="seed-knowledge"><div class="seed-knowledge-tags">${l.tags.map(t=>`<span>${esc(t)}</span>`).join('')}</div><h4>农时参考 · 尚非当前播种规则</h4><dl class="seed-facts"><dt>适播</dt><dd>${l.seasons.join('、')} · ${esc(l.region)}</dd><dt>收获</dt><dd>${esc(l.harvest)}</dd><dt>水土</dt><dd>${esc(l.water)}</dd><dt>题材</dt><dd>${CHINESE_ERA_REFERENCE[l.stage].name} · ${esc(l.period)}</dd></dl><div class="seed-reading-tabs" role="group" aria-label="种子知识">${[['history','历史背景'],['story','图鉴故事'],['science','科学与发展']].map(([id,name])=>`<button type="button" data-atlas-lore-tab="${id}" aria-pressed="${loreTab===id}">${name}</button>`).join('')}</div><div data-atlas-lore-panel="history"><p>${esc(l.history)}</p><details class="seed-references"><summary>参考书目与考证线索</summary><ul>${l.references.map(r=>`<li>${esc(r)}</li>`).join('')}</ul><p>书目用于后续考证；传播日期、古名对应与特定品种仍应核对具体版本和地方资料，不将首次记载等同首次种植。</p></details></div><div data-atlas-lore-panel="story" hidden><span class="atlas-badge">原创故事线索 · 未触发的剧情设想</span><h4>${esc(l.story.title)}</h4><p>${esc(l.story.text)}</p></div><div data-atlas-lore-panel="science" hidden><p>${esc(l.science)}</p><div class="seed-hooks"><h4>后续玩法与剧情的生成点</h4><strong>观察</strong><p>${esc(l.hooks.observation)}</p><strong>选择</strong><p>${esc(l.hooks.choice)}</p><strong>发展</strong><p>${esc(l.hooks.consequence)}</p></div><small>这些是设计线索，尚非行动效果。故事不会直接决定游戏产量或奖励。</small></div></section>`;
}
