import {assetUrl} from './illustration.js';
...
function atlasArt(x:Entry,detail=false):string {
 ...
 return x.art?`<img src="${assetUrl(x.art)}" alt="${detail?esc(x.name)+' · 手绘示意':''}" loading="lazy">`:`<span class="atlas-specimen" aria-hidden="true">${esc(x.name.slice(0,1))}</span>`;
}
