...
function village(g:Game,events:string[]):string {
  ...
  const history=events.length?(encounter?[encounter,...events.filter(t=>t!==encounter).slice(-2)]:events.slice(-3)):['两位同门，从修道习术开始。'];
