...
import {artUrl,itemImage,chromeImage,sectImage} from './illustration.js';
...
const OWN_SPREAD=new Set(['农业','聚落','家人','修炼','学科','制造','系统','雇佣','商城','能源','社会','家业','作坊','副本','试炼']);
...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const nav=`<nav class="chapter-nav" aria-label="篇章" style="--chapter-count:${groups.length}">${groups.map(x=>`<button type="button" class="chapter-tab" data-page="${x.pages.includes(page)?page:x.pages[0]}" aria-current="${x===group?'page':'false'}" title="${esc(x.description)}">${g.sect&&['师徒','修炼'].includes(x.name)?sectImage(x.name==='师徒'?0:1,'sect-navigation-picture'):chromeImage('nav-'+navigationArt[x.name],navigationArt[x.name]==='society'?'chapter-image chapter-image-society':'chapter-image')}<span>${esc(x.name)}</span></button>`).join('')}</nav>
...
function sectPage(g:Game,button:Button):string {
 ...
 const lineage=x.current.map((id,slot)=>{
   ...
   return `<article class="sect-lineage ${active?'is-active':''}"><div class="sect-lineage-head"><span class="sect-seal">${slot===0?'壹':'贰'}</span><span>第 ${m.generation} 代 · ${active?'当前行动者':'同门传人'}</span></div>${sectImage(15,'sect-person-picture')}<h3>${esc(m.name)}</h3><p class="subtle">${m.life?.ageYears??'—'} 岁 · ${m.life?.alive?'在世':'已故'} · ${esc(m.life?.talent.name??'')}</p><div class="sect-stats"><span><strong>${m.stage}</strong>境界</span><span><strong>${m.time}</strong>时间</span><span><strong>${m.life?.energy??'—'}</strong>精力</span></div>${meter('个人修为',m.practice,r.stageProgress*r.maxStage)}<p class="subtle">本道减耗 ${m.effect}%</p>${active?'<p class="sect-active-label">● 正在以此人行动</p>':button('economy:sectswitch:'+slot)}<div class="sect-disciple">${sectImage(disciple?14:13,'sect-disciple-picture')}<span class="eyebrow">↓ 师承相续</span><h4>${disciple?esc(disciple.name):'虚席待徒'}</h4><p>${disciple?`${disciple.life?.ageYears??'—'} 岁 · ${disciple.life?.alive?'在世':'已故'} · 修为 ${disciple.practice}`:'寻访有缘人，再由当前传人收为弟子。'}</p></div></article>`;
 }).join('');
 return `<div class="sect-banner"><div class="sect-art sect-art-teaching" aria-hidden="true"></div><div><span class="eyebrow">道以传志 · 术以济世</span><h3>两脉相承，一志不移</h3><p>每位传人各收一徒，双方弟子成年后整代交接。</p><span class="tag">当前由 ${esc(current.name)} 行动 · 换人不刷新时间</span></div></div><div class="sect-workbench"><section><div class="sect-lineages">${lineage}</div>
 ${panel('寻访与收徒',`${sectImage(13,'sect-panel-picture')}${x.members.filter(m=>!m.admitted).map(m=>`<div class="sect-candidate"><h3>候选 · ${esc(m.name)}</h3><p>${m.life?.ageYears}岁 · ${esc(m.life?.talent.name??'')} · 精力上限${m.life?.maxEnergy}；资质已固定</p></div>`).join('')||'<p class="subtle">先寻访，再查看候选资质并决定是否收徒。</p>'}<div class="sect-action-row">${button('economy:sectseek:disciple')}${button('economy:sectadmit:disciple')}</div>`)}
 ${panel('师徒谱系',`${sectImage(7,'sect-panel-picture')}<details data-fold="sect-lineage-history"><summary>查看历代门人 · ${members.length} 人</summary><ul class="check-list">${members.map(m=>`<li>第${m.generation}代 ${esc(m.name)} · ${m.life?.ageYears}岁 · ${m.life?.alive?'在世':'已故'} · 师父${esc(members.find(p=>p.id===m.masterId)?.name??'开派传人')} · 修为${m.practice}</li>`).join('')}</ul></details>`)}
 </section><aside>${panel('传承交接',`${sectImage(11,'sect-panel-picture')}<p>${esc(successionState(g))}</p><p class="subtle">传下心法与志向，弟子仍须亲自修道习术。</p>${button('economy:retire:family')}${button('handover')}`)}${panel('身体与术的传授',`${sectImage(12,'sect-panel-picture')}${button('economy:rest:self')}${button('economy:care:self')}<p class="subtle">术须逐门学习，可在学堂教导弟子，也可向在世师父请教。</p>${g.actions.filter(a=>a.id.startsWith('economy:consult:')).map(a=>button(a.id)).join('')}`)}${panel('本门心法',`${sectImage(10,'sect-panel-picture')}<p>心法 ${x.doctrine}/${r.doctrineMax} 重 · 跨代保留</p><button type="button" data-page="修炼">前往静修与研证 →</button>`)}</aside></div>`;
}
