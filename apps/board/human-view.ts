...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const hud=calendar&&l?(()=>{
    ...
    const stat=(label:string,value:string,detail:string)=>`<div class="overview-stat"><span>${label}</span><strong>${value}</strong><small>${detail}</small></div>`;
    ...
    return `<section class="journey-overview" aria-label="日历与当前状态"><section class="lunar-month" aria-label="${esc(calendar.month.name)}日历"><header><div><small>第${calendar.year}年 · ${calendar.yearName}年</small><strong>${esc(calendar.month.name)}</strong></div><span>${esc(calendar.lunarDate)} · ${calendar.period}</span></header><div class="month-weekdays" aria-hidden="true">${['日','一','二','三','四','五','六'].map(d=>`<span>${d}</span>`).join('')}</div><div class="month-grid">${'<span class="month-blank"></span>'.repeat(calendar.month.offset)}${days}</div></section><div class="overview-main"><div class="overview-resources" aria-label="当前资源">${stat('精力',`${l.budget.freeEnergy}<em> / ${l.person.maxEnergy}</em>`,'可用 / 当前上限')}${stat('粮食可支持',`${l.diet?.days??0}<em> 天</em>`,l.diet?.name??'简单饮食')}${stat('钱财',`${g.family.money}`,'可用于补给与建设')}${stat('本季可用',`${l.budget.freeTime}<em> 天</em>`,`预留劳动 ${l.budget.reservedTime} 天`)}</div><div class="overview-environment"><section class="overview-weather" aria-label="当前天气"><img src="/illustrations/weather-${g.world.weather}-v1-ui.webp" alt="" width="280" height="280"><div><h3>${calendar.weather.name}<small>降雨供水 ${calendar.weather.rain}</small></h3><p>${esc(calendar.weather.effect)}</p><span>约 ${calendar.weather.days} 天后变化</span></div></section><section class="overview-term" aria-label="节气与节日"><span class="term-season">${calendar.season}</span><div><h3>${esc(calendar.currentTerm)}${calendar.festivals.map(f=>`<small class="today-festival" title="${esc(f.description)}">${esc(f.name)}</small>`).join('')}</h3><p>${esc(calendar.termDescription)}</p><div class="overview-upcoming">${calendar.upcoming.slice(0,2).map(d=>`<span title="${esc(d.date)}">${esc(d.name)} <b>${d.days}天后</b></span>`).join('')}</div></div></section></div><div class="overview-notice" aria-label="最近节气见闻">${latest?`<span>${esc(latest.term)}见闻</span><strong>${esc(latest.title)}</strong><b class="${latest.positive?'gain':'cost'}">${esc(latest.effect)}</b><p title="${esc(latest.date)} · 已结算">${esc(latest.text)}</p>`:`<span>田间消息</span><p>下一节气「${esc(calendar.nextTerm?.name??'')}」会有新的见闻，日常劳作经过当天也会触发。</p>`}</div></div></section>`;
  })():'';
