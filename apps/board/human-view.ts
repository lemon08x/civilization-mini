...
function costChips(g:Game, a:Game['actions'][number]):string {
  ...
  return `<span>${time} 天</span>${g.life?`<span>精力 ${a.energy??0}</span>`:''}${a.money?`<span>钱 ${a.money}</span>`:''}${a.food?`<span>粮 ${a.food}</span>`:''}`;
}
...
export function confirmContent(g:Game, id:string):{title:string;body:string;submitLabel:string} {
  if(g.sect&&id==='economy:erasettle:stage'&&g.era){
    ...
    return {title:modern?'结束现代使命':'结束当前文明阶段',submitLabel:modern?'确认结束旅程':'确认结算并进入下一阶段',body:`<p>当前阶段：<strong>${esc(e.stage.name)}</strong>。</p><p>${modern?'结束本季并结束整个旅程，按已完成的使命计分。':'结束本季，结算当前阶段回报并进入下一文明阶段。师徒、个人修为与所学继续保留。'}</p>${!modern?`<p>当前剩余 <strong>${e.timeBudget.remaining} 季</strong>（含本季），结算本季后剩余 ${e.timeBudget.transferable} 季。</p><p>按 ${e.timeBudget.percent}% 向下取整：下一阶段增加 <strong>${e.timeBudget.carry} 季</strong>，损耗 ${e.timeBudget.lost} 季。</p><p>已取得回报可兑现 ${e.expectedReward} 钱；本季实际新增收益另计，未度过的时间不再折现。师徒交接不重置倒计时。</p>`:''}${e.crises?`<p>剩余 ${e.crises.remaining} 季 · 当前总分 ${e.crises.score}。</p><p>尚未兜底：${e.crises.entries.filter(x=>x.level<1).map(x=>esc(x.name)).join('、')||'无'}。提前结束将立即判定使命成败。</p>`:''}`};
  }
...
function personPortrait(p:{portrait:string;portraitEra:number;ageYears:number;alive:boolean;sex?:string;character?:{temperament:string}|null},name:string,fullBody=false):string {
  ...
  if(temperament>=0&&p.ageYears>=12&&p.portraitEra<2&&(p.sex==='male'||p.sex==='female')){
    ...
    if(fullBody)return `<div class="self-fullbody${p.alive?'':' is-deceased'}" role="img" aria-label="${esc(name)} · ${esc(p.character!.temperament)} · ${['少年','成年','老年'][age]}全身立绘" style="background-image:url('/illustrations/sect-fullbody-${p.sex}.png');background-position:${temperament*20}% ${age*50}%"></div>`;
...
function attention(g:Game,page:string):string {
  ...
  if(l)return `<h3>日历余量</h3><p>本季可用天数 ${l.budget.freeTime} / ${l.timePerSeason}<br>可用精力 ${l.budget.freeEnergy}</p>`;
...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const masthead=`<header class="chronicle-masthead"><div><div class="eyebrow">CIVILIZATION MINI</div><h1>隐士修所 <span>师徒传承</span></h1><p class="rule-line">规则 ${esc(g.rulesVersion)}${g.era?` · ${esc(g.era.framework.name)}`:''}</p></div><div class="masthead-meta"><div class="masthead-stage"><span>${l?esc(g.person.name):'当前家族'}${g.era?` · ${esc(g.era.stage.name)} · 剩余 ${g.era.timeBudget.remaining} 季`:''}</span>${stageExit}</div><div class="masthead-links"><button type="button" class="text-btn" data-page="图鉴" aria-current="${page==='图鉴'?'page':'false'}">${chromeImage('nav-study','utility-image')}<span>图鉴</span></button><details class="options-menu"><summary class="text-btn">${chromeImage('status-save','utility-image')}<span>选项</span></summary><div class="options-panel"><a href="/start">打开其他旅程</a><a href="/start#legacy" target="_blank" rel="noopener">门派缘起与传承之志 ↗</a><a href="/">入口</a><a href="/ai">AI 文本玩法</a><button type="button" id="export">导出存档</button><label class="file-button">导入存档<input id="import" type="file" accept=".json,application/json" hidden></label><p class="subtle">结果由固定规则结算；浏览器存档在开始界面打开或新开，命令行存档在 saves/。</p></div></details><button type="button" class="text-btn" data-guide="${esc(page)}">${chromeImage('status-help','utility-image')}<span>帮助</span></button></div></div></header>`;
  const navigationArt:Record<string,string>={本季:'season',农场:'farm',我:'family',道:'study',术:'study',集市:'market',经营与交易:'trade',社会历程:'society'};
  const nav=`<nav class="chapter-nav" aria-label="篇章" style="--chapter-count:${groups.length}">${groups.map(x=>`<button type="button" class="chapter-tab" data-page="${x.pages.includes(page)?page:x.pages[0]}" aria-current="${x===group?'page':'false'}" title="${esc(x.description)}">${x.name==='我'&&g.life?`<span class="self-nav-portrait">${personPortrait(g.life.person,g.person.name)}</span>`:stageModules[x.name]?`<img class="chapter-image era-nav-image" src="/illustrations/era-nav-${stageModules[x.name].art}.webp" alt="" width="96" height="96">`:g.sect&&['我','道'].includes(x.name)?sectImage(x.name==='我'?0:1,'sect-navigation-picture'):chromeImage('nav-'+navigationArt[x.name],navigationArt[x.name]==='society'?'chapter-image chapter-image-society':'chapter-image')}<span>${esc(x.name)}</span>${stageModules[x.name]?`<small class="era-nav-stage">${(g.era?.index??0)<stageModules[x.name].stage?['一阶段','二阶段','三阶段','四阶段'][stageModules[x.name].stage]:stageModules[x.name].stage===3?'筹备中':'生产已开放'}</small>`:''}</button>`).join('')}</nav>
  ...
  const hud=calendar&&l?(()=>{
    ...
    return `<section class="journey-overview" aria-label="日历与当前状态"><section class="lunar-month" aria-label="${esc(calendar.month.name)}日历"><header><div><small>第${calendar.year}年 · ${calendar.yearName}年</small><strong>${esc(calendar.month.name)}</strong></div><span>${esc(calendar.lunarDate)} · ${calendar.period}</span></header><div class="month-weekdays" aria-hidden="true">${['日','一','二','三','四','五','六'].map(d=>`<span>${d}</span>`).join('')}</div><div class="month-grid">${'<span class="month-blank"></span>'.repeat(calendar.month.offset)}${days}</div></section><div class="overview-main"><div class="overview-resources" aria-label="当前资源">${stat('energy','精力',`${l.budget.freeEnergy}<em> / ${l.person.maxEnergy}</em>`,'可用 / 当前上限')}${stat('food','粮食可支持',`${l.diet?.days??0}<em> 天</em>`,l.diet?.name??'简单饮食')}${stat('money','钱财',`${g.family.money}`,'可用于补给与建设')}${stat('time','本季可用',`${l.budget.freeTime}<em> 天</em>`,`预留劳动 ${l.budget.reservedTime} 天`)}</div><div class="overview-environment"><section class="overview-weather" aria-label="当前天气"><img src="/illustrations/weather-${g.world.weather}-v1-ui.webp" alt="" width="280" height="280"><div><h3>${calendar.weather.name}<small>降雨供水 ${calendar.weather.rain}</small></h3><p>${esc(calendar.weather.effect)}</p><span>约 ${calendar.weather.days} 天后变化</span></div></section><section class="overview-term" aria-label="节气与节日"><span class="overview-season-art"><img src="/illustrations/almanac-${seasonArt}-v1-ui.webp" alt="" width="88" height="88" decoding="async"><span class="term-season">${calendar.season}</span></span><div><h3>${esc(calendar.currentTerm)}${calendar.festivals.map(f=>`<small class="today-festival" title="${esc(f.description)}">${esc(f.name)}</small>`).join('')}</h3><p>${esc(calendar.termDescription)}</p><div class="overview-upcoming">${calendar.upcoming.slice(0,2).map(d=>`<span title="${esc(d.date)}">${esc(d.name)} <b>${d.days}天后</b></span>`).join('')}</div></div></section></div><div class="overview-notice" aria-label="最近节气见闻"><img class="overview-notice-art" src="/illustrations/farm-affairs-story.webp" alt="" width="30" height="30" decoding="async">${latest?`<span>${esc(latest.term)}见闻</span><strong>${esc(latest.title)}</strong><b class="${latest.positive?'gain':'cost'}">${esc(latest.effect)}</b><p title="${esc(latest.date)} · 已结算">${esc(latest.text)}</p>`:`<span>田间消息</span><p>下一节气「${esc(calendar.nextTerm?.name??'')}」会有新的见闻，日常劳作经过当天也会触发。</p>`}</div></div></section>`;
  })():'';
...
function sectPage(g:Game,button:Button):string {
 const x=g.sect!,me=x.members.find(m=>m.id===x.activeId)!,r=x.rules,p=me.life!,l=g.life!,e=g.economy!;
 ...
 if(end)notices.push({title:'这一段旅程已结束',detail:endingCopy(g),action:'<a href="/start">回到开始页 →</a>'});
  ...
  if(foodWarning(g))notices.push({title:'先准备这一季的口粮',detail:`可食储备 ${e.foodTotal} 份，当前库存与预计购粮尚不足以覆盖季耗。`,action:link('农业','去买补给','market')});
  ...
  if(l.budget.freeEnergy<2)notices.push({title:'精力不多了',detail:`可用精力 ${l.budget.freeEnergy}，休息可以恢复精力，但会占用时间。`,action:button('economy:rest:self')});
  if(ready)notices.push({title:`${ready} 块田地可收获`,detail:'作物已经成熟，去田地安排收成。',action:link('农业','去收获','field')});
  if(l.pendingRetirement)notices.push({title:'已安排季末交接',detail:'结束本季后，再确认由弟子接续。',action:''});
 }
 const personDetail=(m:typeof me)=>`<article class="self-person-detail">${m.life?personPortrait(m.life,m.name):''}<h4>${esc(m.name)}</h4><p>${m.life?.ageYears??'—'} 岁 · ${m.life?.alive?'在世':'已故'} · 修为 ${m.practice}</p>${characterCard(m)}</article>`;
 return `<div class="self-home self-visual-home">
 <div class="self-visual-opening"><section class="self-hero" aria-label="我的当前状态"><div class="self-identity"><span class="eyebrow">${l.calendar.date} · ${esc(g.world.weatherName)}</span><h3>${esc(me.name)}</h3><p>第 ${me.generation} 代传人 · ${p.ageYears} 岁 · ${esc(p.character?.occupation??'门中传人')}</p><span class="tag">${esc(p.character?.temperament??'门中传人')}</span><span class="tag" title="${esc(p.talent.effect)}">天赋 · ${esc(p.talent.name)}</span><p class="self-mood">${esc(p.character?.mood??'')}</p><div class="self-personal-progress"><span>境界 <strong>${['未入境','定心','观照','知止','守一','通明'][me.stage]??me.stage}</strong></span><span>修为 <strong>${me.practice}/${r.stageProgress*r.maxStage}</strong></span></div></div><div class="self-portrait">${personPortrait(p,me.name,true)}</div><div class="self-vitals">${meter('健康',p.health,p.maxHealth??100,'health')}${meter('精力',l.budget.freeEnergy,p.maxEnergy,'energy')}${meter('本季余日',l.budget.freeTime,l.timePerSeason)}<small>已过 ${l.budget.spentTime} 天 · 预留 ${l.budget.reservedTime} 天 / ${l.budget.reservedEnergy} 精力</small></div></section>
 </div>
 <div class="self-season-strip"><span>可食储备 <strong>${e.foodTotal}</strong> 份</span><span>饮食可支持 <strong>${l.diet?.days??0}</strong> 天</span><span>钱财 <strong>${g.family.money}</strong></span><span>本门心法 <strong>${x.doctrine}/${r.doctrineMax}</strong> 重</span><span>修行减耗 <strong>${me.effect}%</strong></span><span>已掌握 <strong>${learned}</strong> 门课程</span></div>
 ...
 <details data-fold="self-story"><summary><img class="self-record-art" src="/illustrations/self-record-story.webp" alt="" width="112" height="88" loading="lazy"><strong>我的经历</strong><span>性情、所学与成长记录</span></summary><div class="self-record-body">${characterCard(me)}<h4>已经掌握的知识</h4><p>${esc(knownNames(g))}</p></div></details>
 <details data-fold="self-rest"><summary><img class="self-record-art" src="/illustrations/self-record-rest.webp" alt="" width="112" height="88" loading="lazy"><strong>身体休养</strong><span>健康 ${p.health} · 精力 ${l.budget.freeEnergy}</span></summary><div class="self-record-body self-action-list">${button('economy:rest:self')}${button('economy:care:self')}</div></details>
 <details data-fold="self-peers"><summary><img class="self-record-art" src="/illustrations/self-record-peers.webp" alt="" width="112" height="88" loading="lazy"><strong>我的同门</strong><span>${peers.map(m=>esc(m.name)).join('、')||'暂无同门'}</span></summary><div class="self-record-body">${peers.map(personDetail).join('')}${link('农业','拜访同门','neighbor')}</div></details>
 <details data-fold="self-disciple"><summary><img class="self-record-art" src="/illustrations/self-record-disciple.webp" alt="" width="112" height="88" loading="lazy"><strong>我的弟子</strong><span>${disciple?esc(disciple.name)+' · '+(disciple.life?.ageYears??'—')+' 岁':'尚未收徒 · 寻访有缘人'}</span></summary><div class="self-record-body">${disciple?personDetail(disciple):'<p>寻访后先了解候选人的资质，再决定是否收徒。</p>'}${candidates.map(personDetail).join('')}<div class="self-action-list">${button('economy:sectseek:disciple')}${button('economy:sectadmit:disciple')}${g.actions.filter(a=>a.id.startsWith('economy:bond:')).map(a=>button(a.id)).join('')}</div></div></details>
 <details data-fold="self-lineage"><summary><img class="self-record-art" src="/illustrations/self-record-lineage.webp" alt="" width="112" height="88" loading="lazy"><strong>我的传承</strong><span>${g.status==='handover'?'等待接续':l.pendingRetirement?'已安排季末交接':x.ready?'弟子已成年，可安排交接':'师承谱系与代际交接'}</span></summary><div class="self-record-body"><p>${esc(successionState(g))}</p><div class="self-action-list">${button('economy:retire:family')}${button('handover')}${g.actions.filter(a=>a.id.startsWith('economy:consult:')).map(a=>button(a.id)).join('')}</div><details data-fold="self-history"><summary>历代门人 · ${members.length} 人</summary>${members.map(personDetail).join('')}</details></div></details></section>
 ${!end&&g.status!=='handover'?`<footer class="self-season"><div><strong>安排好这一季，再向前</strong><span>季末会结算生活消耗、生产与恢复。</span></div>${button('economy:end:season')}</footer>`:''}
 </div>`;
}
