...
function successionState(g:Game):string {
  if(g.sect)return g.status==='handover'?'自己的弟子已可接班，确认后开启下一代。':g.sect.ready?'自己的弟子已成年，可安排交接。':'你可以收一名弟子，成年后交接；邻居独立生活，不参与交接。';
...
function village(g:Game,events:string[]):string {
  ...
  const history=events.length?(encounter?[encounter,...events.filter(t=>t!==encounter).slice(-2)]:events.slice(-3)):['你与邻居，在村中各自经营。'];
