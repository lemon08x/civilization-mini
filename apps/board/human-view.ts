...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const navigationArt:Record<string,string>={本季:'season',农场:'farm',我:'family',修炼:'study',学习与制造:'study',集市:'market',经营与交易:'trade',社会历程:'society'};
...
function sectPage(g:Game,button:Button):string {
 ...
 return `<div class="self-home">
 <section class="self-hero" aria-label="我的当前状态"><div class="self-portrait">${personPortrait(p,me.name)}<span>第 ${me.generation} 代传人</span></div><div class="self-identity"><span class="eyebrow">第 ${l.calendar.year} 年 · ${l.calendar.season} · ${esc(g.world.weatherName)} · ${esc(g.era?.stage.name??'旅途中')}</span><h3>${esc(me.name)}</h3><p>${p.ageYears} 岁 · ${esc(p.character?.occupation??'门中传人')} · ${esc(p.character?.temperament??'')}</p><p class="self-mood">${esc(p.character?.mood??'这一季，仍有新的可能。')}</p><span class="tag">天赋 · ${esc(p.talent.name)}</span><span class="self-talent">${esc(p.talent.effect)}</span></div><div class="self-vitals">${meter('健康',p.health,p.maxHealth??100,'health')}${meter('可用精力',l.budget.freeEnergy,p.maxEnergy,'energy')}${meter('可用时间',l.budget.freeTime,l.timePerSeason)}<small>已用 ${l.budget.spentTime} 时间 · 预留 ${l.budget.reservedTime} 时间 / ${l.budget.reservedEnergy} 精力</small></div></section>
 <div class="self-main"><section class="self-focus"><div class="self-section-title"><h3>此刻，先做什么</h3><span>${notices.length?'需要留意 '+notices.length+' 项':'本季概览'}</span></div>${notices.length?notices.map(n=>`<article class="self-notice"><div><h4>${n.title}</h4><p>${esc(n.detail)}</p></div>${n.action}</article>`).join(''):`<div class="self-clear"><strong>眼前没有急需处理的事项</strong><p>还有 ${l.budget.freeTime} 时间、${l.budget.freeEnergy} 精力。可以照料田地，或为自己学一点新本领。</p></div>`}<div class="self-provisions"><span>可食储备 <strong>${e.foodTotal}</strong> 份</span><span>本季消耗 <strong>${g.parameters.foodPerTurn}</strong> 份</span><span>钱财 <strong>${g.family.money}</strong></span></div></section>
 <section class="self-growth"><div class="self-section-title"><h3>我的成长</h3><span>道与术</span></div><div class="self-growth-numbers"><div><strong>${['未入境','定心','观照','知止','守一','通明'][me.stage]??me.stage}</strong><span>当前境界 · 减耗 ${me.effect}%</span></div><div><strong>${learned}<small> 门</small></strong><span>已掌握课程</span></div></div>${meter('修为',me.practice,r.stageProgress*r.maxStage)}<p class="subtle">本门心法 ${x.doctrine} / ${r.doctrineMax} 重 · 跨代保留</p><div>${link('修炼','去修炼')}${link('学科','去学习')}</div></section></div>
 <section class="self-destinations" aria-label="我的事务入口"><button type="button" data-page="农业" data-destination="field">${chromeImage('nav-farm','self-entry-art')}<span><strong>我的经营</strong><small>${fields.length} 块田地 · ${ready?ready+' 块可收获':'照料与探索'}</small></span><b>→</b></button><button type="button" data-page="农业" data-destination="home">${sectImage(12,'self-entry-art')}<span><strong>日常起居</strong><small>烹饪、休息与生活安排</small></span><b>→</b></button>${(g.era?.index??0)>0?`<button type="button" data-page="${['','工坊','贸易','资本'][Math.min(3,g.era!.index)]}"><img class="self-entry-art" src="/illustrations/era-nav-${['','workshop','trade','capital'][Math.min(3,g.era!.index)]}.webp" alt=""><span><strong>当下事业</strong><small>${esc(g.era!.stage.name)} · 玩法筹备中</small></span><b>→</b></button>`:''}</section>
 <section class="self-records"><div class="self-section-title"><h3>我的故事与牵挂</h3><span>需要时，再展开</span></div>
 <details data-fold="self-story"><summary><strong>我的经历</strong><span>性情、所学与成长记录</span></summary><div class="self-record-body">${characterCard(me)}<h4>已经掌握的知识</h4><p>${esc(knownNames(g))}</p></div></details>
 <details data-fold="self-rest"><summary><strong>身体休养</strong><span>健康 ${p.health} · 精力 ${l.budget.freeEnergy}</span></summary><div class="self-record-body self-action-list">${button('economy:rest:self')}${button('economy:care:self')}</div></details>
 <details data-fold="self-peers"><summary><strong>我的同门</strong><span>${peers.map(m=>esc(m.name)).join('、')||'暂无同门'}</span></summary><div class="self-record-body">${peers.map(personDetail).join('')}${link('农业','拜访同门','neighbor')}</div></details>
 <details data-fold="self-disciple"><summary><strong>我的弟子</strong><span>${disciple?esc(disciple.name)+' · '+(disciple.life?.ageYears??'—')+' 岁':'尚未收徒 · 寻访有缘人'}</span></summary><div class="self-record-body">${disciple?personDetail(disciple):'<p>寻访后先了解候选人的资质，再决定是否收徒。</p>'}${candidates.map(personDetail).join('')}<div class="self-action-list">${button('economy:sectseek:disciple')}${button('economy:sectadmit:disciple')}${g.actions.filter(a=>a.id.startsWith('economy:bond:')).map(a=>button(a.id)).join('')}</div>${link('学科','去学堂传授知识')}</div></details>
 <details data-fold="self-lineage"><summary><strong>我的传承</strong><span>${g.status==='handover'?'等待接续':l.pendingRetirement?'已安排季末交接':x.ready?'弟子已成年，可安排交接':'师承谱系与代际交接'}</span></summary><div class="self-record-body"><p>${esc(successionState(g))}</p><div class="self-action-list">${button('economy:retire:family')}${button('handover')}${g.actions.filter(a=>a.id.startsWith('economy:consult:')).map(a=>button(a.id)).join('')}</div><details data-fold="self-history"><summary>历代门人 · ${members.length} 人</summary>${members.map(personDetail).join('')}</details></div></details></section>
