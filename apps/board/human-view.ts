...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const masthead=`<header class="chronicle-masthead"><div><div class="eyebrow">CIVILIZATION MINI</div><h1>隐士修所 <span>师徒传承</span></h1><p class="rule-line">规则 ${esc(g.rulesVersion)}${g.era?` · ${esc(g.era.framework.name)}`:''}</p></div><div class="masthead-meta"><div class="masthead-stage"><span>${l?esc(g.person.name):'当前家族'}${g.era?` · ${esc(g.era.stage.name)} · 剩余 ${g.era.timeBudget.remaining} 季`:''}</span>${stageExit}</div><div class="masthead-links"><details class="options-menu"><summary class="text-btn">${chromeImage('status-save','utility-image')}<span>选项</span></summary><div class="options-panel"><a href="/start">打开其他旅程</a><a href="/start#legacy" target="_blank" rel="noopener">门派缘起与传承之志 ↗</a><a href="/">入口</a><a href="/ai">AI 文本玩法</a><button type="button" id="export">导出存档</button><label class="file-button">导入存档<input id="import" type="file" accept=".json,application/json" hidden></label><p class="subtle">结果由固定规则结算；浏览器存档在开始界面打开或新开，命令行存档在 saves/。</p></div></details><button type="button" class="text-btn" data-guide="${esc(page)}">${chromeImage('status-help','utility-image')}<span>帮助</span></button></div></div></header>`;
  const navigationArt:Record<string,string>={本季:'season',农场:'farm',我:'family',修炼:'study',学习:'study',集市:'market',经营与交易:'trade',社会历程:'society'};
  const nav=`<nav class="chapter-nav" aria-label="篇章" style="--chapter-count:${groups.length}">${groups.map(x=>`<button type="button" class="chapter-tab ${x.name==='我'?'chapter-tab-self':''}" data-page="${x.pages.includes(page)?page:x.pages[0]}" aria-current="${x===group?'page':'false'}" title="${esc(x.description)}">${x.name==='我'&&g.life?`<span class="self-nav-portrait">${personPortrait(g.life.person,g.person.name)}</span>`:stageModules[x.name]?`<img class="chapter-image era-nav-image" src="/illustrations/era-nav-${stageModules[x.name].art}.webp" alt="" width="96" height="96">`:g.sect&&['我','修炼'].includes(x.name)?sectImage(x.name==='我'?0:1,'sect-navigation-picture'):chromeImage('nav-'+navigationArt[x.name],navigationArt[x.name]==='society'?'chapter-image chapter-image-society':'chapter-image')}<span>${esc(x.name)}</span>${stageModules[x.name]?`<small class="era-nav-stage">${(g.era?.index??0)<stageModules[x.name].stage?'第 '+(stageModules[x.name].stage+1)+' 阶段开放':stageModules[x.name].stage===3?'筹备中':'生产已开放'}</small>`:''}</button>`).join('')}</nav>
...
function sectPage(g:Game,button:Button):string {
 ...
 <div class="self-visual-opening"><section class="self-hero" aria-label="我的当前状态"><div class="self-identity"><span class="eyebrow">第 ${l.calendar.year} 年 · ${l.calendar.season} · ${esc(g.world.weatherName)}</span><h3>${esc(me.name)}</h3><p>第 ${me.generation} 代传人 · ${p.ageYears} 岁 · ${esc(p.character?.occupation??'门中传人')}</p><span class="tag">${esc(p.character?.temperament??'门中传人')}</span><span class="tag" title="${esc(p.talent.effect)}">天赋 · ${esc(p.talent.name)}</span><p class="self-mood">${esc(p.character?.mood??'')}</p><div class="self-personal-progress"><span>境界 <strong>${['未入境','定心','观照','知止','守一','通明'][me.stage]??me.stage}</strong></span><span>修为 <strong>${me.practice}/${r.stageProgress*r.maxStage}</strong></span></div></div><div class="self-portrait">${personPortrait(p,me.name,true)}</div><div class="self-vitals">${meter('健康',p.health,p.maxHealth??100,'health')}${meter('精力',l.budget.freeEnergy,p.maxEnergy,'energy')}${meter('时间',l.budget.freeTime,l.timePerSeason)}<small>已用 ${l.budget.spentTime} 时间 · 预留 ${l.budget.reservedTime} 时间 / ${l.budget.reservedEnergy} 精力</small></div></section>
 </div>
 <div class="self-season-strip"><span>可食储备 <strong>${e.foodTotal}</strong> 份</span><span>本季消耗 <strong>${g.parameters.foodPerTurn}</strong> 份</span><span>钱财 <strong>${g.family.money}</strong></span><span>本门心法 <strong>${x.doctrine}/${r.doctrineMax}</strong> 重</span><span>修行减耗 <strong>${me.effect}%</strong></span><span>已掌握 <strong>${learned}</strong> 门课程</span></div>
