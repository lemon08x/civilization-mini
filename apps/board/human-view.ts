...
function sectPage(g:Game,button:Button):string {
 ...
 const scene=(index:number)=>`<span class="self-scene self-scene-${index}" aria-hidden="true"></span>`;
 const tile=(index:number,page:string,title:string,detail:string,destination='')=>`<button type="button" class="self-picture-tile" data-page="${page}" ${destination?`data-destination="${destination}"`:''}>${scene(index)}<span class="self-tile-caption"><strong>${title}</strong><small>${esc(detail)}</small></span><span class="self-tile-arrow" aria-hidden="true">↗</span></button>`;
 return `<div class="self-home self-visual-home">
 <div class="self-visual-opening"><section class="self-hero" aria-label="我的当前状态"><div class="self-identity"><span class="eyebrow">第 ${l.calendar.year} 年 · ${l.calendar.season} · ${esc(g.world.weatherName)}</span><h3>${esc(me.name)}</h3><p>${p.ageYears} 岁 · ${esc(p.character?.occupation??'门中传人')}</p><span class="tag">${esc(p.character?.temperament??'门中传人')}</span><span class="tag" title="${esc(p.talent.effect)}">天赋 · ${esc(p.talent.name)}</span></div><div class="self-portrait">${personPortrait(p,me.name,true)}<span>第 ${me.generation} 代传人</span></div><div class="self-vitals">${meter('健康',p.health,p.maxHealth??100,'health')}${meter('精力',l.budget.freeEnergy,p.maxEnergy,'energy')}${meter('时间',l.budget.freeTime,l.timePerSeason)}<small>已用 ${l.budget.spentTime} 时间 · 预留 ${l.budget.reservedTime} 时间 / ${l.budget.reservedEnergy} 精力</small></div></section>
 <section class="self-picture-grid" aria-label="我的事务入口">${tile(0,'农业','耕作与收获',`${fields.length} 块田地 · ${ready?ready+' 块可收获':'去照料田地'}`,'field')}${tile(1,'修炼','静室修行',`${['未入境','定心','观照','知止','守一','通明'][me.stage]??me.stage} · 修为 ${me.practice}/${r.stageProgress*r.maxStage}`)}${tile(2,'学科','读书习术',`已掌握 ${learned} 门课程`)}${tile(3,'农业','日常起居','烹饪 · 休息 · 生活安排','home')}</section></div>
 <div class="self-season-strip"><span>可食储备 <strong>${e.foodTotal}</strong> 份</span><span>本季消耗 <strong>${g.parameters.foodPerTurn}</strong> 份</span><span>钱财 <strong>${g.family.money}</strong></span><span>本门心法 <strong>${x.doctrine}/${r.doctrineMax}</strong> 重</span><span>修行减耗 <strong>${me.effect}%</strong></span>${(g.era?.index??0)>0?link(['','工坊','贸易','资本'][Math.min(3,g.era!.index)],'当下事业 · '+g.era!.stage.name):''}</div>
 ...
 <section class="self-records"><div class="self-section-title"><h3>人在山中，事在心上</h3><span>点开画卷，查看详情与行动</span></div>
 <details data-fold="self-story"><summary>${scene(2)}<strong>我的经历</strong><span>性情、所学与成长记录</span></summary><div class="self-record-body">${characterCard(me)}<h4>已经掌握的知识</h4><p>${esc(knownNames(g))}</p></div></details>
 <details data-fold="self-rest"><summary>${scene(3)}<strong>身体休养</strong><span>健康 ${p.health} · 精力 ${l.budget.freeEnergy}</span></summary><div class="self-record-body self-action-list">${button('economy:rest:self')}${button('economy:care:self')}</div></details>
 <details data-fold="self-peers"><summary>${scene(4)}<strong>我的同门</strong><span>${peers.map(m=>esc(m.name)).join('、')||'暂无同门'}</span></summary><div class="self-record-body">${peers.map(personDetail).join('')}${link('农业','拜访同门','neighbor')}</div></details>
 <details data-fold="self-disciple"><summary>${scene(5)}<strong>我的弟子</strong><span>${disciple?esc(disciple.name)+' · '+(disciple.life?.ageYears??'—')+' 岁':'尚未收徒 · 寻访有缘人'}</span></summary><div class="self-record-body">${disciple?personDetail(disciple):'<p>寻访后先了解候选人的资质，再决定是否收徒。</p>'}${candidates.map(personDetail).join('')}<div class="self-action-list">${button('economy:sectseek:disciple')}${button('economy:sectadmit:disciple')}${g.actions.filter(a=>a.id.startsWith('economy:bond:')).map(a=>button(a.id)).join('')}</div>${link('学科','去学堂传授知识')}</div></details>
 <details data-fold="self-lineage"><summary>${scene(5)}<strong>我的传承</strong><span>${g.status==='handover'?'等待接续':l.pendingRetirement?'已安排季末交接':x.ready?'弟子已成年，可安排交接':'师承谱系与代际交接'}</span></summary><div class="self-record-body"><p>${esc(successionState(g))}</p><div class="self-action-list">${button('economy:retire:family')}${button('handover')}${g.actions.filter(a=>a.id.startsWith('economy:consult:')).map(a=>button(a.id)).join('')}</div><details data-fold="self-history"><summary>历代门人 · ${members.length} 人</summary>${members.map(personDetail).join('')}</details></div></details></section>
