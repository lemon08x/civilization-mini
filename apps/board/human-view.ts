...
function attention(g:Game,page:string):string {
  ...
  if(l)return `<h3>日历余量</h3><p>本阶段可用天数 ${l.budget.freeTime}<br>可用精力 ${l.budget.freeEnergy}</p>`;
...
export function humanScreen(g:Game,page:string,body:string,button:Button,events:string[],guide=false,recap:Recap|null=null):string {
  ...
  const navigationArt:Record<string,string>={近况:'season',农场:'farm',我:'family',道:'study',术:'study',集市:'market',经营与交易:'trade',社会历程:'society'};
  ...
  const budget=l?`<details class="budget-details"><summary>查看劳动安排 · 预留 ${l.budget.reservedTime} 天 / ${l.budget.reservedEnergy} 精力</summary><p>已过 ${l.budget.spentTime} 天 · 阶段剩余 ${l.timeRemaining} 天</p>${l.budget.tasks.map(t=>`<p>${esc(t.name)} <span class="tag">${t.time} 天 / 精力 ${t.energy}</span></p>`).join('')||'<p>暂无系统任务</p>'}</details>`:'';
...
function sectPage(g:Game,button:Button):string {
 ...
 <div class="self-visual-opening"><section class="self-hero" aria-label="我的当前状态"><div class="self-identity"><span class="eyebrow">${l.calendar.date} · ${esc(g.world.weatherName)}</span><h3>${esc(me.name)}</h3><p>第 ${me.generation} 代传人 · ${p.ageYears} 岁 · ${esc(p.character?.occupation??'门中传人')}</p><span class="tag">${esc(p.character?.temperament??'门中传人')}</span><span class="tag" title="${esc(p.talent.effect)}">天赋 · ${esc(p.talent.name)}</span><p class="self-mood">${esc(p.character?.mood??'')}</p><div class="self-personal-progress"><span>境界 <strong>${['未入境','定心','观照','知止','守一','通明'][me.stage]??me.stage}</strong></span><span>修为 <strong>${me.practice}/${r.stageProgress*r.maxStage}</strong></span></div></div><div class="self-portrait">${personPortrait(p,me.name,true)}</div><div class="self-vitals">${meter('健康',p.health,p.maxHealth??100,'health')}${meter('精力',l.budget.freeEnergy,p.maxEnergy,'energy')}${meter('阶段余日',l.budget.freeTime,g.era?.timeBudget.limit??l.timePerSeason)}<small>已过 ${l.budget.spentTime} 天 · 预留 ${l.budget.reservedTime} 天 / ${l.budget.reservedEnergy} 精力</small></div></section>
 </div>
