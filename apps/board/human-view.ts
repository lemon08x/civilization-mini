...
export function cultivationPage(g:Game,button:Button,selected=''):string {
 ...
 const node=(type:string,lv:number,name:string,state:string,sub:string,known:boolean)=>`<button type="button" class="cultivation-node ${known?'known':''} ${picked===type+':'+lv?'selected':''}" data-cultivation="${type}:${lv}" aria-pressed="${picked===type+':'+lv}"><span class="cultivation-number">${lv.toString().padStart(2,'0')}</span><strong>${name}</strong><small>${state}</small><small>${sub}</small></button>`;
...
function sectPage(g:Game,button:Button):string {
 ...
 </section><aside>${panel('修道与传承',`<p>${esc(current.name)} · 第${current.stage}境 · 修为${current.practice}/${r.stageProgress*r.maxStage}</p><p>本门心法${x.doctrine}/${r.doctrineMax}级。个人修为亲自积累，心法改进跨代保留。</p><button type="button" data-page="修炼">打开修炼图 →</button>`)}</aside></div>`;
}
