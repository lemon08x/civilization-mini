...
export function farm(g:FarmGame,renderButton:(id:string)=>string,manufacturing=''):string {
 ...
 if(panel==='field'){
  ...
  let actions='',extra='',hint='',careInfo='';
  ...
  if(p.kind==='field'&&f){
   ...
   fieldTags.push(f.crop?(f.growth>=f.duration?'成熟待收':`距收获 ${p.maturity?.days??0} 天`):'等待播种',`肥力 ${f.fertility}/3`,`补水 ${f.moisture}`,p.id===map.homeId?'起始田 · 自动供水':'扩展田 · 手动照料');
   ...
   if(f.crop&&f.growth<f.duration){
    const water=g.world.rain+f.moisture+p.waterSupport,need=Math.max(0,2-water);
    ...
    careInfo=`<div class="farm-water-guide"><div class="farm-water-heading"><strong>${need?'本季还缺 '+need+' 点水分':'本季水分已足'}</strong><span>灌溉用于防止缺水减产</span></div><div class="farm-water-flow"><span>降雨 <b>${g.world.rain}</b> ＋ 已补水 <b>${f.moisture}</b> ＋ 地景 <b>${p.waterSupport}</b> ＝ <b>${water}</b></span><span>生长所需 <b>2</b></span><span>${need?`灌溉补 <b>${gain}</b> → 达到 <b>${water+gain}</b>${water+gain<2?'，仍缺 '+(2-water-gain):'，水分足够'}`:'无需继续灌溉'}</span></div><p>每次耗 1 公共水${lift?'，并消耗提水器耐用':''}；每块田每季只能亲自灌溉一次。水分不足 2 的天数会累计受灾，影响最终收成（最低 1）。</p><small>补水季末清零；灌溉不加快成熟，也不会消除已有受灾。${p.id===map.homeId?'此处只计当前水分，起始田安排的供水在推进日历时按设备与任务执行。':''}${need&&water+gain<2?'一次灌溉仍不足，可查看同门浇水是否可用。':''}</small>${offer&&!offer.enabled?`<p class="farm-water-blocked">当前无法灌溉：${esc(offer.reason??'暂不可用')}</p>`:''}</div>`;
   }
