...
export function erasPage(o:SessionObservation,button:(id:string)=>string):string{
 ...
 const reading=`<p>回看这一阶段的积累，选择何时进入下一社会。每个时代有独立的家族，新时代从无亲属关系的新随机人物开始。现代阶段结算后，旅程结束。</p>
   ${clock}
   ...
   <section class="panel"><div class="panel-head"><h2>知识开放</h2></div><div class="panel-body"><p>部分课程要社会发展到相应阶段才出现在学堂。个人知识只属于本时代的人物；新时代角色从基础学习，生产规程与公共记录保留。</p><p>尚未开放：${(e.lockedKnowledge??[]).map(n=>esc(n.name)).join('、')||'当前社会课程均已开放'}</p></div></section>`;
 const settle=`<h3>结算当前社会</h3>
   <p>${modern?'提交后会结束本季、结束这次旅程，并按副本任务计分。':`提交后会结束本季并进入下一社会，由新随机人物开始；剩余约 ${e.remaining} 季（按代际估计）将推算兑现。`}预估会受当季实际结果影响，最终金额以真实结算事件为准。</p>
   <ul class="check-list">
