...
export function erasPage(o:SessionObservation,button:(id:string)=>string):string{
 ...
 +panel('本阶段生产与回报',`<p>已取得回报凭证${e.rewardClaims}份。现在结算预计兑现${e.expectedReward}钱（含剩余${p.remaining}季推算${p.claims}份）。每${e.rewardDivisor}凭证兑换1钱，向下取整。</p><p>每份真实收获计${e.stage.foodWeight}份基础凭证，每份加工产出计${e.stage.craftWeight}份。推算按已建系统、公共供水和人员约束，不掷未来天气。</p><p>${p.notes.map(esc).join('；')||'尚未形成可推算的产能。'}</p>${e.electricRewardPercent!==undefined?`<p>${esc(e.electricRequirement??'')} 当前回报比例 ${e.electricRewardPercent}%。预览已计入该比例；结算以本季实际用电为准。</p>`:''}<p>本阶段收益结清后不被后续公共服务追溯取消。</p>${o.eraSettlements?.map(x=>`<p>${esc(x.detail)}</p>`).join('')||'<p>尚无已结束的社会阶段。</p>'}`)
 +panel('知识开放',`<p>部分课程要社会发展到相应阶段才出现在学堂。已经学会的知识继续生效。</p><p>尚未开放：${(e.lockedKnowledge??[]).map(n=>esc(n.name)).join('、')||'当前社会课程均已开放'}</p>`)
 +(e.dungeon?panel('最终副本：跨代家业试炼',`<p>工程进度${e.dungeon.progress}/${e.dungeon.target} · ${e.dungeon.complete?'已完成':'可混合农业、机械、资金或数学路线'}</p><p>不是离开现代的条件。完成副本并结算后才算旅程胜利。</p>${button('economy:dungeonstart:family')}${e.dungeon.powered!==undefined?`<p>通电验收：${e.dungeon.powered?'已真实交付电力':'尚未完成；进度达标后仍须交付电力'}</p>`:''}${['food','craft','contract','math','power','appliance'].map(x=>button('economy:dungeonwork:'+x)).join('')}`):panel('最终副本','<p>只在现代社会开放。前面各阶段的生产回报独立结算。</p>'));
}
