...
const paintedCrops=new Set([...modeledCrops,'radish','waxgourd']);
let destroyed=false;
...
function loadEnvironment():Promise<boolean>{
 ...
 environmentLoad=Promise.all(['landscape','tree','rock','house','brush','unknown'].map(name=>new Promise<boolean>(resolve=>{
  const image=new Image();
...
function groundTexture(field:boolean,land?:{soil:string;water:number}|null){
 ...
 if(landImage){const f=landFrames[frame];return texture('land-cartoon:'+frame,g=>{g.drawImage(landImage!,f.x,f.y,f.w,f.h,21,110,138,77);});}
...
function unknownTexture(){const image=environmentImages.get('unknown');if(image)return texture('painted:unknown',g=>g.drawImage(image,21,110,138,77));return texture('unknown',g=>{polygon(g,[[90,110],[159,145],[90,180],[21,145]],'#bfcbb7');polygon(g,[[90,110],[159,145],[90,165],[21,145]],'#e7eddf');});}
...
function treeTexture(){return paintedProp('tree',160,156)??texture('tree',(g)=>{ellipse(g,86,159,32,11,'#46643b24');stroke(g,[[86,153],[88,68]],'#7a7652',7);stroke(g,[[87,122],[67,87]],'#7a7652',4);const blobs:[number,number,number,number,string][]=[[64,96,29,28,'#58776bb5'],[106,100,27,29,'#6c8c77b8'],[82,73,34,35,'#779984c4'],[71,68,22,24,'#9db395bd'],[102,80,20,25,'#819e89bf']];for(const b of blobs)ellipse(g,...b);for(let i=0;i<26;i++){let x=57+(i*17%58),y=55+(i*13%56);ellipse(g,x,y,2+(i%3),1.5,'#b9c78c50');}});}
function rockTexture(){return paintedProp('rock',132,76)??texture('rock',g=>{polygon(g,[[40,158],[51,136],[86,124],[112,137],[128,158]],'#8a9184');polygon(g,[[51,136],[86,124],[95,146],[66,150]],'#b1b29b');});}
function brushTexture(){return paintedProp('brush',136,78)??texture('brush',g=>{for(let k=0;k<7;k++){const x=48+k*14;stroke(g,[[x,164],[x-8,118+k%3*5]],'#71854c',3);stroke(g,[[x,164],[x+10,126+k%2*8]],'#8da05c',3);}});}
...
function cropTexture(crop:string,growth:number){
 ...
 if(image)return texture('painted:'+crop+stage,g=>{
  const trailing=crop==='waxgourd',leaf=crop==='mallow'||crop==='mustard';
  const spots=trailing?[[76,145],[106,156]]:leaf?[[76,143],[105,156]]:[[80,137],[104,147],[79,157]];
  const height=(trailing?[20,39,47]:crop==='radish'?[19,44,52]:leaf?[18,40,48]:[22,62,73])[stage];
  for(let i=0;i<spots.length;i++){
   const [x,y]=spots[i],scale=Math.min(height/image.naturalHeight,(trailing?67:leaf?54:51)/image.naturalWidth)*(1+(i-1)*.035);
...
return {landscapeTexture,mistTexture,groundTexture,unknownTexture,houseTexture,treeTexture,rockTexture,brushTexture,cropTexture,loadCrops,loadLand,loadEnvironment,sprite,destroy:()=>{destroyed=true;landImage=null;landFrames.length=0;cropImages.clear();cropLoads.clear();environmentImages.clear();for(const t of textureCache.values())t.destroy(true);textureCache.clear();}};
}
