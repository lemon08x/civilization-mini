...
const KEY = 'civilization-mini.rules-lab.v5';
...
const ruleset = validateRuleset(await (await fetch('/rulesets/social-inheritance.v4.json')).json());
...
function render() {
  ...
  $('rule-notice').textContent = `规则 ${o.rulesVersion} · ${o.society ? '个人、家族与社会 / 跨行业传承' : o.technologyOutcomes ? '科技改变生活 / 设施、批量制作与传承' : o.production ? '跨地域生产 / 木作、陶作、储存与农业' : '旧农业基线'} · 每代八季是实验窗口，数值仍待检验`;
  ...
  $('app').innerHTML = `
    ...
    <div class="workspace"><div>
      ...
      <section class="panel"><div class="panel-head"><h2>世界与行动</h2><small>行动结果来自同一规则引擎</small></div><div class="panel-body">
        ...
        ${!end && o.status !== 'handover' ? `<nav class="tabs" aria-label="行动类型">${(o.production ? ['获取', '制作', '保存', '交换', '生活', '学习', '实践', '设施', '研究', '传承', ...(o.society ? ['社会'] : [])] : ['生活', '学习', '实践', '设施', '研究', '传承']).map(item => `<button data-group="${item}" class="${group === item ? 'selected' : ''}">${item}</button>`).join('')}</nav>
