...
let selectedTechStage=-1,selectedManufactureStage=-1;
...
$('import').onchange=async ev=>{const file=(ev.target as HTMLInputElement).files?.[0];if(!file||busy)return;busy=true;try{const value=JSON.parse(await file.text());const next=parseSession(value);if(next.record.manifest.ruleset.rulesVersion!==rules.rulesVersion)throw new Error('仅导入当前规则存档');const id=crypto.randomUUID();writeSave(id,next,`导入 · ${new Date().toLocaleString()}`);location.assign('/play?save='+encodeURIComponent(id));}catch(e){error((e as Error).message);busy=false;}};
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-page]').forEach(b=>b.onclick=()=>{page=b.dataset.page!;if(page==='学科'){selectedTechStage=-1;selectedCourse='';}if(page==='制造'){selectedManufactureStage=-1;selectedProduct='';}if(b.dataset.destination)selectFarmPanel(b.dataset.destination as FarmPanel);guide=false;filter='';render();});
  const showSelection=()=>{
    render();
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-manufacture-stage]').forEach(b=>b.onclick=()=>{selectedManufactureStage=Number(b.dataset.manufactureStage);selectedProduct='';render();});
  document.querySelectorAll<HTMLButtonElement>('[data-product]').forEach(b=>{const go=()=>{selectedProduct=b.dataset.product!;selectedManufactureStage=observeSession(session).game.economy!.industryView?.catalog.find(p=>p.id===selectedProduct)?.unlockStage??-1;showSelection();};b.onclick=go;if(b.tagName!=='BUTTON')b.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go();}};});
...
function render():void {
  ...
  if(page==='制造')body=e.industryView?manufacturePage(o,button,selectedProduct,selectedManufactureStage):manufactureFallback(o,button);
