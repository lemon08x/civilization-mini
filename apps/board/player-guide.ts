...
export function ruleGuide(g:SessionObservation['game'],page:string):string {
  ...
  const guides:Record<string,[string,string][]>={
    社会:[['何时结算','前三阶段的原有代际额度折算为公开季数倒计时，换代不重置。提前结束先正常结算本季，剩余完整季数按50%向下取整加入下一阶段。只兑现实际取得的回报，不再推算未来收益。现代期限也可接收结转，旅程结束不再结转。水井和泵不是过关条件。'],['社会服务','每个时代提供不靠科技树的公共服务：农耕村落有公地采食和帮工，市镇百工有公井和公共磨坊，电力工业有铁/电工市场，现代有配送和自来水。'],['科技开放','部分课程要社会发展后才出现。个人知识不跨时代复制；新人物从基础学习，生产规程与公共记录保留。'],['收益归属','凭证只在本阶段兑现。后期自来水不会追溯取消前期水井的价值。最终副本只影响现代阶段的胜负，不是离开现代的条件。']],
