...
export const pageNames:Record<string,string>={
  图鉴:'图鉴',工坊:'工坊',贸易:'贸易',资本:'资本',社会:'社会历程',聚落:'经营近况',农业:'农场',生活:'补给与谋生',家人:'人物',修炼:'修行',仓库:'仓库',安排:'安排与计划',
  学科:'学艺',制造:'制造',系统:'生产系统',雇佣:'人员安排',商城:'集市',
...
export function playerGroups(g:SessionObservation['game']):PlayerGroup[] {
  ...
  const groups:PlayerGroup[]=[
    {name:'人物',pages:['家人',...(g.sect?['修炼']:[]),'学科'],description:'人物状态、修行与学艺',epigraph:'从此刻的自己，安排下一步。'},
...
export function chapterEpigraph(g:SessionObservation['game'],page:string):string {
  if(page==='修炼')return '静以修身，学以致用。';
  if(page==='学科')return '学以致用，回到田间与工坊实践。';
...
export function ruleGuide(g:SessionObservation['game'],page:string):string {
  ...
  if(g.sect&&page==='修炼')return `<section class="rules-page"><button class="text-btn" data-page="修炼">← 返回修行</button><h2>修行课程</h2><p>从「修身」入门，分为养身、养心、明理、处世四支，再会通为知常、成己成物与道法自然。名称借用古典思想，前置关系与习练内容是门中编排。</p><p>每门课须先修成全部前课。精修花费真实时间与压力，完成本次投入后增加进度，可分段接续；修成的课程不因功课中断而遗失。</p><p>日课用于温习：每次补充${g.sect.rules.upkeepGain}天功课，最多${g.sect.rules.upkeepMax}天，随日历消耗。精修完成本次投入后也将功课储备补满。</p><p>课程效果暂留空，目前不增加行动效率，也不发放旧境界、心法或机缘奖励。实际花费以行动报价为准。</p><p>师父只能传授自己修成的课；弟子也须满足前置，双方都投入时间和压力。交接保留弟子本人学习进度与功课状态，不复制师父所学。</p></section>`;
  if(g.sect&&['家人','社会'].includes(page))return `<section class="rules-page"><button class="text-btn" data-page="${page}">← 返回</button><h2>${pageNames[page]} · 规则</h2><p>你只控制自己，另一位是独立同门，可在农场拜访。自己的弟子成年后即可交接，同门不参与传承。</p><p>个人从「修身」开始学习修行课程，精修与日课需要真实时间。师父可传授自己修成的课，师徒双方都占用时间；修行效果暂待设计。</p><p>跨时代由师徒接续，地块、资产与计划保留；弟子带着自己已经学会的课程和实际进度接手。</p><p>全年计划产值评分尚未接入。现有现代终局仍按旧危机规则结算，后续再衔接新主线；它不是新的修行课程目标。</p></section>`;
  const guides:Record<string,[string,string][]>={
    ...
    学科:[['学堂有什么用','知识是研发与建设的前置。学会知识后，再去产品研发取得相应产品；学习本身不会赠送设备。'],['怎样学习','先展开一门课程，检查前置、效果与当前报价，再点击学习。当前学习只消耗时间并增加压力，不消耗样品。费用受天赋和传承影响，以行动按钮为准。'],['分叉而不是单链','力学、材料、农学、组织都从根节点分出并列分支。学田间水分不是学肥力或油料作物的前提；学交付不必先开工序。数学在基础之后分成应用数学与账目计量。'],['社会开放课程','成形、泵、发电等要社会发展后才开放。农业就能学的是栽培、储存、基础力学和劳动分工。跨时代保留人物与已学知识；新入门弟子仍须逐项学习。'],['三种状态','当前可学：已满足知识前置，仍需足够时间和口粮。待前置：先学指定课程。已掌握：可以查看记录与教导选项。'],['知识怎样传承','弟子仍需按前置逐项学习。师承前代学过的知识会降低学习成本；教导可以让后辈提前掌握。留存记录不等于自动学会。'],['产品与知识的区别','实物、产品验证、制造规程分别记录。买到实物不会自动获得制造规程；具体要求在农场、工坊或贸易主场景查看。']],

    家人:[['养育决定起点','后辈未成年前，每一季吃饱、被陪伴、被教导都会记入养育记录；成年那一季按记录一次性结算体质加成，每个后辈出生时独立随机获得天赋并终身保留；教导帮助后辈学习，不会重抽天赋。性别不影响能力，每个人都能从事全部活动。姓名与面貌在接班后保留；肖像随年龄变化；跨时代会开启新的随机人物与家族，旧人物留作历史记录。陪伴行动在家人页，每季一次。'],['时间与压力',`农历有大小月与闰月，按立春、立夏、立秋、立冬换季；行动推动日历，可跨季进行；换季只改变物候，压力仅在主动放松时降低，饱食可缓慢恢复健康，系统劳动仍占用时间。`],['恢复与健康',`花时间放松降低压力，疗养恢复健康。饱食随日期缓慢恢复身体；持续缺粮会加重损伤。健康上限随年龄变化。`],['长辈请教','交接后退休长辈仍在世时，可以请教其掌握的课程：本人下一次学习该课程的时间减少。每门课程每代一次，交接后重新计算；长辈离世后不可再请教。'],['交接','在满足条件时准备交接，再由后辈接手。同一时代由后辈接班；跨时代则换成无亲属关系的新随机人物。实物与公共记录保留，个人知识仍需学习。']],
    ...
    系统:[['运行顺序','建设 → 实际调试 → 安排本人或员工 → 运行。安排人员后查看劳动预留；取消安排可释放预算。'],['为什么停机','系统会检查材料、水源、设备、人力和工资；不满足条件时待命。查看每个系统的当前阻碍。'],['劳动与成本',`员工按实际任务结算工资${e.industryView?`，每 ${e.industryView.rules.wageTimeUnit} 工作时间计 1 钱，单任务向上取整`:''}。本人劳动占用时间与压力。供水按需执行，轴加工每季一批。`],['自动化范围','当前仍需本人播种、收获、采购和交付。外部供粮与维修不代表自有系统无人运行。']],
    商城:[['采购','选择商品加入清单，检查总价后整单结账；购买不消耗时间、不增加压力。'],['到货','现货即刻使用，订货下一季到场。留意当前库存与运输限制。'],['交付与生活','交付会扣除真实物资。预留家庭口粮、工资与下一轮生产需要的材料。']],
    ...
    生活:[['社会食品','在安排页选择自给优先、市场生活或保留储备。采购按预算、资金、共享库存与运输部分执行；暂停自动购买后仍可手动买粮。购买不占个人时间；自动购粮按实际预算与供应执行。'],['零存粮生活','家中没有粮不等于缺粮；查看预计购粮与生活缺口。工资、其他开支和本季采购会改变报价，按日以实际条件结算。'],['谋生与补给','通过当前开放的采集、做工和补给行动维持生活。每项行动都有实际时间、压力和资源成本。'],['行动受限','条件不足的按钮会显示原因。需要恢复时到家人页；需要买粮时到集市。']],
    ...
    能源:[['供能','本季供能需要本人执行；启用设备不等于已经发电。安装、启用和本季实际运行是三种状态。'],['余电与储能','余电可存入已启用的储能设备；未储存的电会在季末耗散。季末不会自动发电。电灯降低工作压力增量；调度仍有天数与压力报价。']],
  };
...
function landGuide(g:SessionObservation['game']):string {
 ...
 const facilities=[
  ...
  {id:'yard',name:'晒场',role:'给赶不及收的田托底',range:4 as const,days:r.yardDays,cost:`${r.projectWood}木材`,effect:'被动生效：正交相邻田正常收获期延长7天，宽限后仍绝收；已累计的受灾不受影响。',site:'建在紧挨重点田的其他用途地上；只护上下左右四格，不含斜角。',choice:'需先掌握场院管理。适合给成熟窗口紧的田兜底。永久占格，建成后不能再种。'},
  ...
  {id:'pit',name:'堆肥坑',role:'把秸秆沤成肥料',range:0 as const,days:r.pitDays,cost:'不耗木材',effect:`在坑格投3份秸秆，${r.pitConvertDays}天后自动转成2份堆肥；转化期间不能再投料，到期按日历自动结算。`,site:'空闲其他用途地即可，建在秸秆来源的田旁更顺手；永久占格。',choice:'需先掌握秸秆堆肥。秸秆不再只是副产品；施肥在农时安排的田间操作中进行。'},
  ...
  {id:'retting',name:'沤麻塘',role:'把亚麻茎沤成纤维',range:0 as const,days:r.rettingDays,cost:`${r.projectWood}木材`,effect:'建成后农场「农具与加工」开放沤麻工序：2份亚麻茎跨季沤为1份亚麻纤维。',site:'空闲其他用途地即可；永久占格。',choice:'需先掌握沤麻脱胶，是亚麻纤维的前置条件；工序报价会提示需建成沤麻塘。'},
 ];
 ...
 <ol class="land-guide-steps"><li><strong>选中一格</strong><p>点击地图或使用“定位地块”。未知格先探索，揭晓后查看土质、地势和水分。</p></li><li><strong>比较周围</strong><p>准备种田，就看能否得到供水、排水；准备建设，就看范围里有多少田、地势是否合适。</p></li><li><strong>选用途与工期</strong><p>清理后的地默认为播种用途。点亮首个地块设施实践后，空闲地可切换为其他用途并建设；工程可以分段完成。</p></li><li><strong>回来看变化</strong><p>日历推进时，天气和设施影响土地。查看水分与渠的通水状态，再决定播种、浇水或收获。</p></li></ol>
 ...
 <p class="land-guide-note"><strong>再进一步：水田。</strong> 掌握田间水分管理后，与水源格或通水渠相邻的播种田可做水田改造（${r.paddyDays}天工程，首次开工需${r.projectWood}木材）；完工后田块水分恒为过湿，水稻免浇水，非耐涝作物不能种。</p></section>
