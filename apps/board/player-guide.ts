...
export function playerGroups(g:SessionObservation['game']):PlayerGroup[] {
  ...
  const groups:PlayerGroup[]=[
    ...
    {name:'学习',pages:['学科'],description:'研习各阶段知识与技艺',epigraph:'学以致用，回到田间与工坊实践。'},
...
export function ruleGuide(g:SessionObservation['game'],page:string):string {
  ...
  const guides:Record<string,[string,string][]>={
    ...
    农业:[['农舍烹饪','收获的小麦、大豆可与木柴做成麦粥、炖豆或麦豆饭。当次消耗食材、时间与精力，产出即食口粮供季末食用；种子不会入锅，亚麻不能做饭。'],['耕作顺序','选择地块 → 探索未知边界 → 开垦 → 播种 → 跨季生长 → 管理或收获。每块田独立结算。持续耕作与自动供水只覆盖起始田。'],['同门往来','同门不可控制，自己的田地和库存独立。可聊天、换种、请教A4或请求灌溉；关系不足或忙碌时会拒绝帮助。'],['发现种子','探索可能发现种囊，辨认后获得豆种或麻种；也可向同门换种。发现后买卖面板开放补购，播种还需A4。异穗麦须选种，市场不出售，收获返还专属种子。'],['未知地块','探索可能遇到荒地、古树、巨石、荆棘、种子与故事。树石不能开垦；荆棘需清理。修渠、分粮与保留田界都有真实成本和结果，每处选择只结算一次。'],['农场事务','仓库、买种买粮、出售、休息与同门在农场页签操作；地图只展示耕作、探索和自家农舍。'],['起始田地',`生长 ${e.field.growth}/${e.field.duration} 季，肥力 ${e.field.fertility}/3，水分补充 ${e.field.moisture}。预计收成随条件变化。`],['种子与口粮','种子单独留存，不作为生活口粮。持续耕作缺种子时停在空田。供水系统只负责起始田的缺水季灌溉。']],
  ...
  const rows=guides[page]??[['一季怎样推进','先安排口粮，再按目标学习或生产。每次行动都会结算成本；结束本季会结算生活消耗、生产和恢复。'],['从哪里开始','本季看板处理当下缺口；家人处理恢复、养育与传承；学习处理知识成长，各阶段主场景处理制造；经营与交易处理补给、安排与买卖。本季纪事只显示当前摘要。']];
