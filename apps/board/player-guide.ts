...
export function playerGroups(g:SessionObservation['game']):PlayerGroup[] {
  ...
  const groups:PlayerGroup[]=[
    ...
    {name:'师徒',pages:['家人'],description:'自己与邻居、收徒与传承',epigraph:'道须亲修，学问相承。'},
  ...
  if((g.era?.index??0)===0)return groups.filter(x=>!['本季','集市','经营与交易'].includes(x.name));
  return groups;
}
...
export function ruleGuide(g:SessionObservation['game'],page:string):string {
  ...
  const guides:Record<string,[string,string][]>={
    ...
    农业:[['耕作顺序','选择地块 → 探索未知边界 → 开垦 → 播种 → 跨季生长 → 管理或收获。每块田独立结算。持续耕作与自动供水只覆盖起始田。'],['邻里往来','邻居不可控制，自己的田地和库存独立。可聊天、换种、请教A4或请求灌溉；关系不足或忙碌时会拒绝帮助。'],['发现种子','探索可能发现种囊，辨认后获得豆种或麻种；也可向邻居换种。发现后买卖面板开放补购，播种还需A4。异穗麦须选种，市场不出售，收获返还专属种子。'],['未知地块','探索可能遇到荒地、古树、巨石、荆棘、种子与故事。树石不能开垦；荆棘需清理。修渠、分粮与保留田界都有真实成本和结果，每处选择只结算一次。'],['农场事务','仓库、买种买粮、出售、休息与邻里在农场页签操作；地图只展示耕作、探索和自家农舍。'],['起始田地',`生长 ${e.field.growth}/${e.field.duration} 季，肥力 ${e.field.fertility}/3，水分补充 ${e.field.moisture}。预计收成随条件变化。`],['种子与口粮','种子单独留存，不作为生活口粮。持续耕作缺种子时停在空田。供水系统只负责起始田的缺水季灌溉。']],
