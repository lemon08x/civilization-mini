...
export const pageNames:Record<string,string>={聚落:'本季总览',农业:'田地',生活:'生活与食品',家人:'家人与传承',仓库:'仓库',学科:'学堂',产品:'产品研发',生产:'加工制作',系统:'生产系统',雇佣:'人员安排',商城:'集市',能源:'供能',家业:'家业',作坊:'作坊',副本:'副本',试炼:'试炼'};
export function playerGroups(g:SessionObservation['game']) {
  const e=g.economy!;
  return [
    {name:'本季总览',pages:['聚落'],description:'先看钱粮与本季安排'},
...
export function ruleGuide(g:SessionObservation['game'],page:string):string {
  ...
  const guides:Record<string,[string,string][]>={
    学科:[['学堂有什么用','知识是研发与建设的前置。学会知识后，再去产品研发取得相应产品；学习本身不会赠送设备。'],['怎样学习','先展开一门课程，检查前置、效果与当前报价，再点击学习。当前学习只消耗时间与精力，不消耗样品。费用受天赋和传承影响，以行动按钮为准。'],['三种状态','当前可学：已满足知识前置，仍需足够时间和精力。待前置：先学指定课程。已掌握：可以查看记录与教导选项。'],['知识怎样传承','后辈仍需按前置逐项学习。直系前代学过的知识会降低学习成本；教导可以让后辈提前掌握。留存记录不等于自动学会。'],['产品与知识的区别','实物、产品验证、制造规程分别记录。买到实物不会自动获得制造规程；具体要求在产品研发中查看。']],
    农业:[['耕作顺序','播种 → 跨季生长 → 管理或收获。点击行动前查看当前条件；水分充足时可能只需等待。'],['当前田地',`生长 ${e.field.growth}/${e.field.duration} 季，肥力 ${e.field.fertility}/3，水分补充 ${e.field.moisture}。预计收成随条件变化。`],['种子与口粮','种子单独留存，不作为生活口粮。播种和收获由本人操作；建设供水系统不代替这些行动。']],
    家人:[['时间与精力',`每季 ${l?.timePerSeason??12} 时间。行动消耗时间与精力，系统任务会预留预算；可用值已经扣除预留。`],['恢复与健康',`休息换取精力，疗养恢复健康。饱食季末恢复身体${l?.recovery?`，当前恢复 ${l.recovery.fedHealth} 健康`:''}；持续缺粮会加重损伤。健康上限随年龄变化。`],['交接','在满足条件时安排季末交接，再由后辈接手。家族实物和记录保留，后辈的个人知识仍需学习。']],
    产品:[['研发顺序','前置知识与产品验证 → 试制或检验 → 验证记录。材料只在制造或检验时投入。'],['三类成果','实物可以使用或消耗；验证记录证明产品已验证；制造规程支持后续制造。购买实物不附赠规程。'],['试运行','泵与发电机首次试制还需实际试运行。详情列出当前缺少的知识、产品与材料。']],
