...
const base=JSON.parse(await readFile('rulesets/social-eras.v23.json','utf8'));
...
const output='artifacts/experiments/social-eras-v23-'+Date.now();await mkdir(output,{recursive:true});
...
for(const seed of [17,29])for(const delay of [0,6]){
 ...
 const settled=events.filter(e=>e.type==='era'&&e.operation==='settled').map(e=>({claims:e.amount/10,money:e.money,detail:e.detail}));
 const projected=events.filter(e=>e.type==='era'&&e.operation==='projected').map(e=>({claims:e.amount/10,detail:e.detail}));
 rows.push({seed,delay,card:session.record.snapshots[0].state.era.card,builtSeason:built,elapsedAtSettle:g.era.index>0?undefined:g.era.elapsed,wellTasks:worked,harvest,projected,stageSettled:settled,missing:events.filter(e=>e.type==='season-settled').reduce((n,e)=>n+e.missing,0),health:g.life.person.health,commands:session.record.entries.length,nextStage:g.era.index});
