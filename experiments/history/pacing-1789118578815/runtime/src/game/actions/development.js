...
export function developmentActions(state, rules) {
    const d = state.development;
    if (!d || !rules.development)
        return [];
    const p = rules.development.parameters, actions = [];
    for (const r of DEVELOPMENT_RECIPES) {
        ...
        ], `${r.benefit} 开工投入${Object.entries(r.inputs).map(([g, n]) => `${n}${GOOD_NAMES[g]}`).join('、') || '原料'}；再用1行动完成${r.wait ? '，需跨季' : ''}。`, (draft, events) => {
    ...
    actions.push(defineAction(state, 'finish-development', '完成工业项目', '工业', {}, [...(!recipe ? ['没有工业在制品'] : []), ...(recipe && state.clock.absoluteTurn < d.project.started + recipe.wait ? ['需跨季干燥或校验'] : [])], '完成已布置工序并获得产品与实践；后辈可接续，不自动授予前代技能。', (draft, events) => {
        ...
        draft.development.goods[r.output] += amount;
        draft.development.project = null;
    ...
    actions.push(defineAction(state, 'conduct-experiment', '开展材料对照实验', '科学', { food: Math.max(0, p.experimentFood - Math.min(1, d.designs.science)) }, [
        ...
        if (draft.productNetwork) {
            draft.productNetwork.goods.spentCeramics++;
            events.push({ type: 'ceramic-residue', amount: 1 });
        }
        const x = draft.development;
        x.goods.supplies--;
        x.goods.ceramicParts--;
        const equipped = x.labDurability > 0;
        if (equipped)
            x.labDurability--;
        const amount = equipped ? 2 : 1;
        x.goods.findings += amount;
        addUnique(activePerson(draft).practices, 'development-experimentation');
    ...
    for (const domain of DISCIPLINES) {
        ...
        actions.push(defineAction(state, `mentor:${domain}`, `带后辈实习：${DISCIPLINE_NAMES[domain]}`, '传承', { food: p.mentorFood }, [...(parentXP < 2 ? ['本人实践积累不足'] : []), ...(childXP >= Math.floor(parentXP / 2) ? ['后辈已达到当前可传授的实践水平'] : [])], '花时间与口粮传授1点熟练经验，最多达到本人一半；不替代知识的理论与实践条件。', (draft, events) => gainExperience(draft, domain, 1, events, draft.household.heirId)));
    }
