...
export function livelihoodActions(state, rules) {
    ...
    return [
        ...
        defineAction(state, 'cultivate', '耕作并记录', '生活', {}, state.location.season.cultivated ? ['本季已耕作'] : [], `本季可得 ${crop.food} 口粮；引水 ${crop.drawn}，试验地占用 ${crop.landCost}。结果由当前天气、种源和设施计算。`, (draft, events) => {
            ...
            if (draft.development && draft.development.fieldDurability > 0 && result.gross > 0) {
                draft.development.fieldDurability--;
                events.push({ type: 'field-equipment-used', remaining: draft.development.fieldDurability });
            }
            sampleTrial(draft, rules, events);
            if (draft.productNetwork && result.storedDrawn) {
                draft.productNetwork.storedWater -= result.storedDrawn;
                events.push({ type: 'stored-water-used', amount: result.storedDrawn });
            }
            draft.household.food += result.food;
            draft.location.water -= result.drawn;
            const asset = channel(draft);
            if (result.drawn > 0 && asset) {
                asset.durability--;
                draft.location.season.usedChannel = true;
            }
            draft.location.season.cultivated = true;
            addUnique(activePerson(draft).practices, 'cultivation');
            if (draft.location.weather === 'dry')
                addUnique(activePerson(draft).insights, 'observation');
            events.push({ type: 'harvest', food: result.food, drawn: result.drawn, deficit: result.deficit, channelExhausted: asset?.durability === 0 });
        }),
        defineAction(state, 'work', '为邻里做工', '生活', {}, state.production?.market.jobs === 0 ? ['本季岗位已用完'] : [], `收入 ${p.workIncome} 钱财。${state.production ? `本季剩余岗位 ${state.production.market.jobs}。` : ''}`, (draft, events) => { if (draft.production)
            draft.production.market.jobs--; draft.household.money += p.workIncome; events.push({ type: 'income', source: 'work', amount: p.workIncome }); }),
        defineAction(state, 'buy-food', `买入 ${purchase} 口粮`, '生活', { money: purchase * p.foodPrice }, state.production && state.production.market.food < purchase ? ['当地可售粮食不足'] : [], `用于家庭生活或实践；交易也占用行动点。${state.production ? `当前市场余粮 ${state.production.market.food}。` : ''}`, (draft, events) => { if (draft.production)
            draft.production.market.food -= purchase; draft.household.food += purchase; events.push({ type: 'food-purchased', amount: purchase }); }),
        defineAction(state, 'sell-food', '卖出 2 口粮', '生活', { food: 2 }, state.production && state.production.market.food + 2 > rules.scenarios[state.location.id].production.market.food ? ['当地粮食收购容量已满'] : [], `收入 ${p.foodPrice} 钱财；不能买卖套利。`, (draft, events) => { if (draft.production)
            draft.production.market.food += 2; draft.household.money += p.foodPrice; events.push({ type: 'income', source: 'sale', amount: p.foodPrice }); }),
        defineAction(state, 'build-channel', '建设家用引水渠', '设施', { ap: 2, money: p.channelCost }, [
            ...
            if (rules.householdProgress) {
                addUnique(activePerson(draft).practices, 'channel-model');
                events.push({ type: 'practiced', personId: activePerson(draft).id, nodeId: 'ditch', practiceId: 'channel-model' });
            }
            const id = 'channel:household-1';
            draft.assets[id] = { id, kind: 'channel', durability: p.channelDurability };
            draft.household.assetIds.push(id);
