...
export function productNetworkActions(s, rules) {
    ...
    for (const r of DEVICE_RECIPES) {
        actions.push(defineAction(s, `fabricate:${r.id}`, `制造${NETWORK_NAMES[r.id]}`, '工业', {}, [
            ...
            ...Object.entries(r.inputs).filter(([id, count]) => productAmount(s, id) < count).map(([id, count]) => `需${count}${PRODUCT_NAMES[id]}`),
        ], `${Object.entries(r.inputs).map(([id, count]) => `${count}${PRODUCT_NAMES[id]}`).join(' + ')}；开工、完成各1行动。${r.effect}`, (draft, events) => {
            for (const [id, count] of Object.entries(r.inputs))
                changeProduct(draft, id, -count);
            draft.productNetwork.project = r.id;
            events.push({ type: 'product-started', device: r.id, inputs: { ...r.inputs } });
        }));
    }
    actions.push(defineAction(s, 'finish-product', '完成高级设备', '工业', {}, n.project ? [] : ['没有高级设备在制品'], '完成已布置的装配；设备需安装后使用，可跨代接续。', (draft, events) => {
        const device = draft.productNetwork.project;
        draft.productNetwork.goods[device]++;
        draft.productNetwork.project = null;
    ...
    for (const device of DEVICES)
        actions.push(defineAction(s, `install-product:${device}`, `安装${NETWORK_NAMES[device]}`, '设施', {}, [
            ...(n.goods[device] < 1 ? ['没有对应设备'] : []), ...(n.installed[device] > 0 ? ['现有设备仍可使用'] : []),
        ], `消耗1件设备，提供${rules.development.parameters.equipmentDurability}次专用操作。继承者可使用，不自动学会制造。`, (draft, events) => {
            const x = draft.productNetwork;
            x.goods[device]--;
            x.installed[device] = rules.development.parameters.equipmentDurability;
    ...
    actions.push(defineAction(s, 'calibrate', '进行仪器校准', '科学', {}, [
        ...
        draft.development.goods.findings--;
        draft.development.goods.supplies--;
        draft.productNetwork.goods.calibrations++;
        draft.productNetwork.installed.calibrator--;
        gainExperience(draft, 'science', 1, events);
    ...
    actions.push(defineAction(s, 'pump-water', '提取并储存公共水', '生活', { materials: { wood: 1 } }, [
        ...
        draft.location.water--;
        draft.productNetwork.storedWater++;
        draft.productNetwork.installed.pump--;
    ...
    actions.push(defineAction(s, 'recycle-ceramics', '回收烧结陶料', '工业', { materials: { wood: 1 } }, [
        ...
        draft.productNetwork.goods.spentCeramics -= 2;
        draft.development.goods.ceramicParts++;
        draft.productNetwork.installed.kiln--;
        gainExperience(draft, 'pottery', 1, events);
        events.push({ type: 'product-operated', device: 'kiln', remaining: draft.productNetwork.installed.kiln, result: 1 });
    }));
    return actions;
}
