...
export function educationActions(state, rules) {
    const actions = [], person = activePerson(state), child = heir(state), p = rules.parameters;
    for (const tech of rules.technologies) {
        ...
        if (tech.prerequisiteAny && !tech.prerequisiteAny.some(id => has(person, id)))
            missing.push(`任选其一：${tech.prerequisiteAny.map(id => techById(rules, id).name).join(' / ')}`);
        ...
        for (const tag of tech.practices) {
            if (tag.startsWith('development-'))
                continue;
            if (['cultivation', 'trial-completed', 'stock-release', 'wood-shaped', 'pot-fired', 'storage-fitted'].includes(tag))
                continue;
