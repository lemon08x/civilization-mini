...
export function initialProduction(scenario) {
...
export function storageCapacity(state, rules) {
...
export function spoilagePreview(state, rules) {
    if (!rules.production)
        return 0;
...
export function spoilFood(state, rules, events) {
    if (!rules.production)
        return;
...
export function methodBlockers(state, rules, id) {
...
export const recipeMethod = (recipe) => recipe === 'pottery' ? 'pottery' : 'woodworking';
export function technologyOutcomes(state, rules) {
    ...
    return {
        ...
        workshops: ['woodenware', 'pottery'].map(material => ({
...
export function gatherPreview(state, rules, resource) {
    const local = state.production, p = rules.production.parameters;
    if (resource === 'food')
        return Math.min(local.stocks.wildFood, p.gatherFood + Number(has(activePerson(state), 'resource-observation')));
    if (resource === 'wood')
        return Math.min(local.stocks.timber, p.gatherWood + (local.toolDurability > 0 ? p.toolBonus : 0));
