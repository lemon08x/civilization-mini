...
export const has = (person, id) => person.mastered.includes(id);
export const prerequisites = (person, tech) => tech.prerequisites.every(id => has(person, id)) && (!tech.prerequisiteAny || tech.prerequisiteAny.some(id => has(person, id)));
export const studyRequired = (rules, person, tech, archives = []) => Math.max(1, tech.study * rules.parameters.studyMultiplier - (rules.socialInheritance && archives.includes(tech.id) ? rules.socialInheritance.archiveDiscount : 0) - Number(person.insights.includes(tech.id)) - Number(tech.helpfulPrerequisites?.some(id => has(person, id)) ?? false));
export function techById(rules, id) {
    const tech = rules.technologies.find(t => t.id === id);
    if (!tech)
        throw new Error(`未知科技：${id}`);
    return tech;
}
export function accessible(state, tech) {
...
export function masterAvailable(state, person, rules, events) {
    let changed;
    do {
        changed = false;
...
export function study(state, nodeId, events) {
...
export function practice(state, nodeId, practiceId, events) {
    const person = activePerson(state);
    addUnique(person.practices, practiceId);
    events.push({ type: 'practiced', personId: person.id, nodeId, practiceId });
}
export function teach(state, nodeId, rules, events, familyLesson = false) {
    ...
    if (kind === 'study')
        child.learning[nodeId] = (child.learning[nodeId] ?? 0) + 1;
    else {
        const tag = tech.practices.find(t => !child.practices.includes(t));
        if (!tag)
            throw new Error('后辈已完成全部实践');
        addUnique(child.practices, tag);
    }
    if (familyLesson && kind === 'study') {
        const tag = tech.practices.find(t => !child.practices.includes(t));
        if (tag) {
            addUnique(child.practices, tag);
            events.push({ type: 'taught', personId: child.id, nodeId, kind: 'practice' });
        }
    }
    if (familyLesson) {
        const domains = { woodworking: 'woodwork', mechanics: 'woodwork', pottery: 'pottery', 'ceramic-engineering': 'pottery', 'precision-engineering': 'pottery', agronomy: 'agriculture', experimentation: 'science' };
        const domain = domains[nodeId];
        if (domain && experience(state, domain, child.id) < Math.floor(experience(state, domain) / 2))
            gainExperience(state, domain, Math.min(2, Math.floor(experience(state, domain) / 2) - experience(state, domain, child.id)), events, child.id);
    }
