...
export function newSeason(state, rules, events) {
    ...
    if (state.development && rules.development) {
        state.development.tradeRemaining = rules.development.parameters.marketSupply;
        state.development.ordersRemaining = rules.development.parameters.marketSupply;
    }
...
export function finishSeason(state, rules, events) {
    ...
    if (family.hardship >= p.hardshipLimit) {
        state.status = 'ended';
        events.push({ type: 'experiment-ended', reason: 'hardship' });
        return;
    }
