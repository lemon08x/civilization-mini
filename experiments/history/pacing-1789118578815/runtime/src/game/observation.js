...
export function projectView(trial) {
...
export function getObservation(state, rules) {
    ...
    return {
        ...(state.productNetwork ? { productNetwork: productNetworkView(state) } : {}),
        ...
        ...(state.society ? { society: { ...structuredClone(state.society), parameters: structuredClone(rules.socialInheritance), contractsView: ['woodenware', 'pottery'].map(material => ({ material, ...structuredClone(state.society.contracts[material]), reason: contractBlocker(state, rules, material) })) } } : {}),
