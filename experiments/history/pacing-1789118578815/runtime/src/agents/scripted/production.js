...
export function chooseProductionAction(observation, policy) {
    ...
    if (policy === 'subsistence')
        return first([...(o.family.food < 6 && o.harvest.food > 0 ? ['cultivate'] : []), 'work', ...(o.family.food < 4 ? ['gather:food'] : []), 'end-turn']);
    ...
    const learn = (id) => {
        ...
        if (mastered(id))
            return null;
        for (const parent of tech.prerequisites)
            if (!mastered(parent))
                return learn(parent);
        if (!tech.accessible)
            return first([`buy-method:${id}`, 'work']);
        if (tech.studied < tech.required)
            return first([`study:${id}`]);
        if (id === 'controlled-fire')
            return first(['practice:controlled-fire:fire-tended', 'gather:wood']);
        return null;
    };
    if (p.project && legal('finish-craft'))
        return 'finish-craft';
    const learning = learn(method);
    if (learning)
        return learning;
    const material = method === 'pottery' ? 'pottery' : 'woodenware';
    if (mastered(method) && !p.storage[material]) {
        if (legal('study:storage'))
            return 'study:storage';
        if (legal(`install-storage:${material}`))
            return `install-storage:${material}`;
    }
    // 留出第一件容器配置储存，后续商品交换获取口粮。
    if (p.inventory[material] > Number(!p.storage[material])) {
        const sale = first([`sell-good:${material}`]);
        if (sale)
            return sale;
    }
    ...
    if (!p.project) {
        if (method === 'woodworking') {
            if (p.inventory.wood < p.parameters.woodRecipeCost)
                return first(['gather:wood', 'work', 'end-turn']);
            if (legal('craft:woodenware'))
                return 'craft:woodenware';
        }
        else {
            if (p.inventory.clay < p.parameters.potteryClayCost) {
                const id = first(['gather:clay']);
                if (id)
                    return id;
            }
            if (p.inventory.wood < p.parameters.potteryFuelCost) {
                const id = first(['gather:wood']);
                if (id)
                    return id;
            }
            if (legal('craft:pottery'))
                return 'craft:pottery';
        }
    }
