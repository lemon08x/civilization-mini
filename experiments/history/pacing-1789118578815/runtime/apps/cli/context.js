...
export async function loadContext() {
    const implementation = await readJson(join(projectRoot, 'dist/implementation.json'));
    ...
    if (hash.digest('hex') !== implementation.codeFingerprint)
        throw new Error('构建产物与代码指纹不匹配，请重新 npm run build');
    ...
    return { implementation, pacingBase, base, developmentBase, societyBase, legacyBase, productionBase, feedbackBase };
}
