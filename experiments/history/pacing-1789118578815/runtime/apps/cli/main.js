...
function options(args) {
    ...
    for (let i = 0; i < args.length; i += 2) {
        if (!args[i].startsWith('--') || args[i + 1] === undefined)
            throw new Error('参数格式：--name value');
        if (Object.hasOwn(result, args[i].slice(2)))
...
运行产物只写 artifacts/。旧 --file 存档请显式 import，不原地覆盖。`);
        ...
        if (opts.rules && !['legacy', 'production', 'feedback', 'society', 'development', 'network', 'pacing'].includes(opts.rules))
            throw new Error('--rules 只支持 legacy / production / feedback / society / development / network / pacing');
        const { implementation, legacyBase } = context, base = opts.rules === 'legacy' ? legacyBase : opts.rules === 'production' ? context.productionBase : opts.rules === 'feedback' ? context.feedbackBase : opts.rules === 'society' ? context.societyBase : opts.rules === 'development' ? context.developmentBase : opts.rules === 'network' ? context.base : context.pacingBase;
        ...
        if (command === 'simulate') {
            ...
            if (!isRecord(raw))
                throw new Error('实验计划必须是对象');
            if (opts.seeds)
                raw.seeds = opts.seeds.split(',').map(Number);
            if (opts.candidate) {
                if (!Array.isArray(raw.variants))
                    throw new Error('计划缺少参数组');
            ...
            try {
                ...
                await writeFile(join(directory, 'failure.json'), JSON.stringify({ error: error.message }), { flag: 'wx' });
                throw error;
            }
        }
        else {
            if (!opts.run)
                throw new Error('请指定 --run；旧文件先使用 import --run 新ID --file 原路径');
            if (command === 'new') {
                ...
                console.log(JSON.stringify(observeSession(session), null, 2));
            }
            else if (command === 'import') {
                if (!opts.file)
                    throw new Error('import 需要 --file');
                const raw = await readJson(resolve(opts.file));
                let session;
                if (isRecord(raw) && raw.format === 'civilization-mini-replay')
                    session = await importLegacy(raw, legacyBase, implementation, opts.run);
                else
                    session = await importRecord(raw, implementation, opts.run);
                await store.create(session);
                console.log(JSON.stringify(observeSession(session), null, 2));
            }
            else if (command === 'act') {
                if (!opts.action || opts.revision === undefined)
                    throw new Error('act 需要 --action 和 --revision');
                ...
                if (command === 'observe')
                    ...
                    console.log(JSON.stringify(metrics(session.record, session.state), null, 2));
                else if (command === 'export')
                    console.log(JSON.stringify(session.record, null, 2));
                else
