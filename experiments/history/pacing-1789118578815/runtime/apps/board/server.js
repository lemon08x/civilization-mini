...
const context = { ...loaded, base: loaded.pacingBase };
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        let absolute;
        if (path === '/')
            absolute = join(projectRoot, 'apps/board/index.html');
        else if (path === '/review')
            absolute = join(projectRoot, 'apps/board/review.html');
        else if (path === '/review.css')
            absolute = join(projectRoot, 'apps/board/review.css');
        else if (path === '/style.css')
            absolute = join(projectRoot, 'apps/board/style.css');
        else if (path === '/implementation.json')
            absolute = join(projectRoot, 'dist/implementation.json');
        else if (['/rulesets/traditional-agriculture.v1.json', '/rulesets/shared-production.v2.json', '/rulesets/technology-feedback.v3.json', '/rulesets/social-inheritance.v4.json', '/rulesets/craft-science.v5.json', '/rulesets/product-network.v6.json', '/rulesets/household-progress.v7.json'].includes(path))
            absolute = join(projectRoot, path.slice(1));
            ...
            if (!absolute.startsWith(root + sep) || absolute.includes(`${sep}tests${sep}`) || absolute.includes(`${sep}apps${sep}cli${sep}`) || absolute.endsWith(`${sep}file-store.js`)) {
                res.writeHead(404);
                res.end();
                return;
            }
