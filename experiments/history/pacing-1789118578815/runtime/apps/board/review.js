...
const show = (v) => v === null || v === undefined ? '未设置 / 无此机制' : typeof v === 'object' ? JSON.stringify(v) : String(v);
const source = (path, label = path.split('/').at(-1)) => `<button data-source="${esc(path)}">${esc(label)}</button>`;
const statuses = { pending_validation: '待验证', pending_decision: '待采纳', accepted: '已接受', rejected: '已拒绝', deferred: '暂不采纳' };
const kinds = { parameter: '参数调整', mechanism: '机制变更', strategy: '策略问题', interface: '界面改进' };
let data, tab = ['technology', 'products', 'parameters', 'research', 'framework'].includes(location.hash.slice(1)) ? location.hash.slice(1) : 'rules', selectedTech = 'woodworking', query = '', scope = 'all', status = 'all', sourceRequest = 0;
...
function currentValue(path) { let value = data.rules; for (const key of path.split('.')) {
...
function experimentCard(e) {
    ...
    const rows = e.results.map(value => { const r = value, m = r.metrics && typeof r.metrics === 'object' ? r.metrics : r; return `<tr><td>${esc(r.runId ?? r.variant ?? '—')}</td><td>${esc(r.scenario ?? r.scenarioId ?? '见清单')}</td><td>${esc(r.agent ?? r.variant ?? '见清单')}</td><td>${esc(m.status ?? '—')}</td><td>${esc(m.money ?? '—')}</td><td>${esc(m.food ?? '—')}</td><td>${esc(m.foodShortfall ?? '—')}</td></tr>`; });
...
function render() {
    ...
    if (researchTools && ['parameters', 'research', 'framework'].includes(tab))
        researchTools.open = true;
    ...
    $('review').innerHTML = data.issues.map(i => `<p class="notice">读取提示：${esc(i)}</p>`).join('') + ({ rules: rulesPage, technology: techPage, products: () => productPage(data.rules, selectedProduct, productCategory, productPlans, productLocal, productHistory.length > 0, productSearch, productCycle), parameters: parametersPage, research: researchPage, framework: frameworkPage }[tab] ?? rulesPage)();
    document.querySelectorAll('[data-knowledge]').forEach(b => b.addEventListener('click', () => { frameworkNode = b.dataset.knowledge; render(); document.querySelector('.knowledge-detail')?.scrollIntoView({ block: 'start' }); }));
    document.querySelectorAll('[data-tab]').forEach(b => b.classList.toggle('selected', b.dataset.tab === tab));
    document.querySelectorAll('[data-tech]').forEach(b => { const select = () => { const graph = document.querySelector('.tech-network'); const left = graph?.scrollLeft ?? 0, top = graph?.scrollTop ?? 0; selectedTech = b.dataset.tech; render(); const next = document.querySelector('.tech-network'); if (next) {
        next.scrollLeft = left;
        next.scrollTop = top;
        if (!b.closest('svg'))
            document.querySelector(`svg [data-tech="${selectedTech}"]`)?.scrollIntoView({ block: 'nearest', inline: 'nearest' });
    } }; b.addEventListener('click', select); b.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        select();
    } }); });
    document.querySelectorAll('[data-source]').forEach(b => b.addEventListener('click', () => void openSource(b.dataset.source)));
    ...
    $('apply-filter')?.addEventListener('click', () => { query = $('query').value; if (tab === 'parameters')
        scope = $('scope').value;
    else
        status = $('status').value; render(); });
    document.querySelectorAll('[data-product-node]').forEach(b => { const select = () => { const graph = document.querySelector('.product-network'), left = graph?.scrollLeft ?? 0, top = graph?.scrollTop ?? 0; rememberProduct(); selectedProduct = b.dataset.productNode; const changedBranch = productCategory !== b.dataset.productCategory; if (productCategory !== b.dataset.productCategory) {
        productCategory = b.dataset.productCategory;
        productCycle = false;
    } render(); const next = document.querySelector('.product-network'); if (next) {
        next.scrollLeft = changedBranch ? 0 : left;
        next.scrollTop = changedBranch ? 0 : top;
        if (!b.closest('svg'))
            document.querySelector(`svg [data-product-node="${selectedProduct}"]`)?.scrollIntoView({ block: 'nearest', inline: 'nearest' });
    } }; b.addEventListener('click', select); b.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        select();
    } }); });
    document.querySelectorAll('[data-product-tech]').forEach(b => b.addEventListener('click', () => { selectedTech = b.dataset.productTech; tab = 'technology'; history.replaceState(null, '', '#technology'); render(); }));
    $('product-category')?.addEventListener('change', () => { rememberProduct(); productCategory = $('product-category').value; productLocal = false; productCycle = false; render(); });
    $('product-mode')?.addEventListener('change', () => { rememberProduct(); productPlans = $('product-mode').value === 'planned'; productCategory = 'all'; productCycle = false; productLocal = false; render(); });
    document.querySelectorAll('[data-product-branch]').forEach(b => b.addEventListener('click', () => { rememberProduct(); productCategory = b.dataset.productBranch; productLocal = false; productCycle = false; render(); }));
    $('product-back')?.addEventListener('click', () => { const p = productHistory.pop(); if (!p)
        return; selectedProduct = p.selected; productCategory = p.category; productPlans = p.plans; productLocal = p.local; productCycle = p.cycle; render(); const g = document.querySelector('.product-network'); if (g) {
        g.scrollLeft = p.left;
        g.scrollTop = p.top;
    } });
    $('product-local')?.addEventListener('click', () => { rememberProduct(); productLocal = !productLocal; render(); });
    $('product-cycle')?.addEventListener('click', () => { rememberProduct(); productCycle = !productCycle; productLocal = false; if (productCycle) {
        selectedProduct = 'g:ceramicParts';
        productCategory = '材料与金属';
    } render(); });
    $('tech-search-form')?.addEventListener('submit', e => { e.preventDefault(); techSearch = $('tech-search').value.trim(); render(); });
    $('product-search-form')?.addEventListener('submit', e => { e.preventDefault(); productSearch = $('product-search').value.trim(); render(); });
    $('query')?.addEventListener('keydown', e => { if (e.key === 'Enter')
        $('apply-filter').click(); });
}
async function openSource(path) {
    const request = ++sourceRequest;
    $('source-title').textContent = path;
    $('source-text').textContent = '正在读取…';
    const dialog = $('source');
    if (!dialog.open)
        dialog.showModal();
    try {
        const response = await fetch(`/review-file?path=${encodeURIComponent(path)}`);
        if (!response.ok)
            throw new Error('文件不可读取或不在允许范围');
        ...
        if (request === sourceRequest)
            $('source-text').textContent = e.message;
    }
}
async function refresh() {
    ...
    try {
        const response = await fetch('/review-data');
        if (!response.ok)
            throw new Error('无法读取审阅数据');
        ...
        message.textContent = `${e.message}。已有显示保留，请重试。`;
...
document.querySelectorAll('[data-tab]').forEach(b => b.addEventListener('click', () => { tab = b.dataset.tab; query = ''; history.replaceState(null, '', tab === 'rules' ? location.pathname : '#' + tab); render(); }));
