...
export function logicPage(r, selected, search = '') {
    ...
    const depth = (id) => { if (depths.has(id))
        return depths.get(id); const n = techs.find(n => n.id === id); const parents = [...n.prerequisites, ...(n.prerequisiteAny ?? [])]; const d = parents.length ? Math.max(...parents.map(depth)) + 1 : 0; depths.set(id, d); return d; };
    const positions = new Map(), occupied = new Map();
    for (const n of [...techs].sort((a, b) => depth(a.id) - depth(b.id))) {
        const d = depth(n.id), used = occupied.get(d) ?? new Set();
        ...
        used.add(row);
        occupied.set(d, used);
        positions.set(n.id, { x: 24 + d * 215, y: 25 + row * 92 });
    }
    ...
    const button = (id) => `<button data-tech="${esc(id)}">${esc(techs.find(n => n.id === id).name)}</button>`;
    const names = (ids) => ids.map(button).join(' ') || '无';
    const { nodes, edges } = buildLogicGraph(r), node = (id) => nodes.find(n => n.id === id);
