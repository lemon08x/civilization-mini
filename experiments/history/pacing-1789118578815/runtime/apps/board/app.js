...
const implementation = await (await fetch('/implementation.json')).json();
...
try {
    if (storageRaw)
        state = await replayRecord(JSON.parse(storageRaw), implementation);
}
catch (failure) {
    loadFailed = true;
    error('原存档未覆盖：' + failure.message);
}
function commit(next, backup = false) {
    if (localStorage.getItem(KEY) !== storageRaw)
        throw new Error('另一页面已更新存档，请刷新后继续');
    if (backup && storageRaw)
        localStorage.setItem(KEY + '.backup.' + Date.now(), storageRaw);
...
async function act(id, revision) {
    if (busy || loadFailed)
        return;
    busy = true;
    try {
        commit((await submitCommand(state, { commandId: state.record.manifest.runId + ':' + revision, expectedRevision: revision, actionId: id })).session);
    }
    catch (failure) {
        render();
...
function render() {
    ...
    document.querySelectorAll('[data-life-category]').forEach(button => button.addEventListener('click', () => { lifeCategory = button.dataset.lifeCategory; render(); }));
    ...
    document.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => void act(button.dataset.action, revision), { once: true }));
    ...
    $('policy-step')?.addEventListener('click', () => { const id = chooseAction(observeSession(state), policy); if (id)
        void act(id, revision); });
}
...
$('new-game').addEventListener('click', async () => {
    ...
    busy = true;
    try {
        commit(await fresh(Number($('seed').value), $('scenario').value), true);
    }
    catch (failure) {
        error(failure.message);
    }
...
$('import').addEventListener('change', async () => {
    ...
    try {
        if (file.size > 5000000)
            throw new Error('存档文件超过 5 MB');
        const raw = JSON.parse(await file.text());
        commit(raw.formatVersion === 1 ? await importLegacy(raw, legacyRuleset, implementation, crypto.randomUUID()) : await importRecord(raw, implementation, crypto.randomUUID()), true);
    }
    catch (failure) {
        error('导入失败，原实验保留：' + failure.message);
    }
    finally {
        busy = false;
        $('import').value = '';
...
function productionPanel(o) {
    ...
    <div class="world-strip">${Object.entries(p.inventory).map(([id, amount]) => `<span class="tag">${MATERIAL_NAMES[id]} ${amount}</span>`).join('')}</div>
...
function lifePanel(o) {
    const categories = { all: '全部经历', life: '生活变化', practice: '手艺与研究', legacy: '家族接续', community: '当地社会' };
...
function productNetworkPanel(o) {
    const n = o.productNetwork;
    if (!n)
        return '';
    const offer = (id) => { const a = o.actions.find(a => a.id === id); return a ? `<button data-action="${escape(a.id)}" data-revision="${o.revision}" ${a.enabled ? '' : 'disabled'}>${escape(a.label)}</button><small>${a.enabled ? escape(a.description) : escape(a.reason)}</small>` : ''; };
...
function developmentPanel(o) {
    const d = o.development;
    if (!d)
        return '';
    const offer = (id) => { const a = o.actions.find(a => a.id === id); return a ? `<button data-action="${escape(a.id)}" data-revision="${o.revision}" ${a.enabled ? '' : 'disabled'}>${escape(a.label)}</button><small>${a.enabled ? `${a.ap}行动 · ${a.money}钱 · ${a.food}粮` : escape(a.reason)}</small>` : ''; };
    ...
    <div class="world-strip">${Object.entries(d.goods).map(([g, n]) => `<span class="tag">${GOOD_NAMES[g]} ${n}</span>`).join('')}</div>
...
function outcomesPanel(o) {
    ...
    <div class="family-grid">${outcomes.workshops.map(workshop => {
        ...
        ${action ? `<p>${action.ap} 行动${Object.entries(action.materials ?? {}).map(([id, amount]) => ` · ${amount} ${MATERIAL_NAMES[id]}`).join('')}</p><button data-action="${escape(action.id)}" ${!action.enabled || loadFailed ? 'disabled' : ''}>${escape(action.label)}</button>${!action.enabled ? `<p>${escape(action.reason)}</p>` : ''}` : ''}</div>`;
    }).join('')}</div>
...
function societyPanel(o) {
    ...
    const button = (id) => {
        ...
        return a ? `<p>${a.ap} 行动${a.money ? ` · ${a.money} 钱财` : ''}${Object.entries(a.materials ?? {}).map(([material, amount]) => ` · ${amount} ${MATERIAL_NAMES[material]}`).join('')}</p><button data-action="${escape(id)}" ${!a.enabled || loadFailed ? 'disabled' : ''}>${escape(a.label)}</button>${!a.enabled ? `<p>${escape(a.reason)}</p>` : ''}` : '';
    };
