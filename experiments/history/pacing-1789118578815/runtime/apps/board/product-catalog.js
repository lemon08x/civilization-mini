...
export function productCatalog(r) {
    const graph = buildLogicGraph(r);
    const groups = { food: ['食品与储存', 0], woodenware: ['食品与储存', 1], pottery: ['食品与储存', 2], storedWater: ['食品与储存', 3], publicWater: ['食品与储存', 0], supplies: ['食品与储存', 2], wood: ['材料与金属', 0], clay: ['材料与金属', 0], ceramicParts: ['材料与金属', 1], mechanisms: ['材料与金属', 2], precisionParts: ['材料与金属', 3], spentCeramics: ['材料与金属', 2], tool: ['工具与设备', 0], fieldTools: ['工具与设备', 1], labTools: ['工具与设备', 1], calibrator: ['工具与设备', 2], pump: ['工具与设备', 3], kiln: ['工具与设备', 3], findings: ['知识成果', 1], calibrations: ['知识成果', 2] };
    const nodes = graph.nodes.filter(n => n.kind === 2).map(n => ({ id: n.id, name: n.name, category: groups[n.id.slice(2)]?.[0] ?? '材料与金属', stage: groups[n.id.slice(2)]?.[1] ?? 0, planned: false, detail: n.id === 'g:findings' ? '由真实材料实验产生，可消耗于工艺改良或仪器校准。不是通用科技点。' : n.id === 'g:calibrations' ? '由校准操作产生，用于提水泵和控温窑制造。不是自动解锁全部科技。' : n.detail }));
    const edges = [];
    const add = (from, to, label, planned = false) => { if (from !== to && !edges.some(e => e.from === from && e.to === to && e.label === label))
        edges.push({ from, to, label, planned }); };
    ...
    for (const process of graph.nodes.filter(n => n.kind === 1)) {
        ...
        for (const input of inputs)
            for (const output of outputs)
                add(input.from, output.to, `${process.name}：${input.label}；${output.label}`);
    }
    ...
    if (r.technologyFeedback) {
        ...
        nodes.push({ id: 'f:workbench', name: '木工作台', category: '工具与设备', stage: 1, planned: false, detail: '掌握木材加工后建造，花' + f.buildActions + '行动、' + f.workbenchWood + '木材；开放木容器批量制作。设施跨代保留。' }, { id: 'f:potteryKiln', name: '陶窑', category: '工具与设备', stage: 1, planned: false, detail: '掌握陶作后建造，花' + f.buildActions + '行动、' + f.kilnWood + '木材、' + f.kilnClay + '黏土；开放陶器批量制作，仍需跨季干燥。' });
        add('g:wood', 'f:workbench', '建造投入' + f.workbenchWood + '木材');
        add('f:workbench', 'g:woodenware', '掌握木作后开放批量制作，仍消耗配方材料');
        add('g:wood', 'f:potteryKiln', '建造投入' + f.kilnWood + '木材');
        add('g:clay', 'f:potteryKiln', '建造投入' + f.kilnClay + '黏土');
        add('f:potteryKiln', 'g:pottery', '掌握陶作后开放批量制作，仍消耗配方材料');
    }
    const plans = [
    ...
    for (const [id, name, category, stage, detail] of plans)
        nodes.push({ id: 'p:' + id, name, category, stage, planned: true, detail });
    for (const [a, b, label] of [
        ...
        ['arithmetic', 'geometry', '计算支持测量'], ['geometry', 'mechanicsModel', '建模支持'], ['mechanicsModel', 'design', '设计依据'], ['thermalModel', 'design', '热加工依据'], ['design', 'furnace', '炉具设计'], ['design', 'mill', '工具设计']
    ])
        add('p:' + a, 'p:' + b, label + '（规划假设）', true);
