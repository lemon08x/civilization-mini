...
async function setup() {
    ...
    async function act(id) { session = (await submitCommand(session, { commandId: `product:${session.record.entries.length}`, expectedRevision: session.record.entries.length, actionId: id })).session; }
    async function doAction(id) {
        for (let i = 0; i < 30; i++) {
            const a = o().actions.find(a => a.id === id);
            if (!a)
                throw new Error(`不存在行动 ${id}`);
            if (o().family.food < 3 && o().actions.find(a => a.id === 'gather:food')?.enabled) {
                await act('gather:food');
                continue;
            }
            if (a.enabled) {
                await act(id);
            ...
            if (o().family.money < a.money && o().actions.find(a => a.id === 'work')?.enabled) {
                await act('work');
                continue;
            }
            const missing = Object.entries(a.materials ?? {}).find(([m, n]) => o().production.inventory[m] < n);
            if (missing && o().actions.find(a => a.id === `gather:${missing[0]}`)?.enabled) {
                await act(`gather:${missing[0]}`);
                continue;
            }
            if (!/跨季|采购机会|木材不足|黏土不足|钱财不足/.test(a.reason))
                throw new Error(`${id}: ${a.reason}`);
            await act('end-turn');
        }
        throw new Error(`等待超限 ${id}`);
    }
    return { get session() { return session; }, get o() { return o(); }, act, doAction, async chain(...ids) { for (const id of ids)
            await doAction(id); } };
}
test('产品网络独立规则，旧规则不注入；图包含分流、汇合与回路', async () => {
    ...
    assert.equal(base.rulesVersion, '0.6.0');
    assert.throws(() => validateRuleset({ ...developmentBase, productNetwork: true }));
...
test('高级产品开启校准、跨季蓄水、回收新操作，消耗守恒且可跨代使用', async () => {
    ...
    assert.equal(g.o.development.goods.precisionParts, 0);
    assert.equal(g.o.development.goods.labTools, 0);
    const before = JSON.stringify(g.session.record);
    await assert.rejects(g.act('fabricate:calibrator'), /在制项目/);
    ...
    for (let i = 0; i < 2; i++)
        await g.chain('procure:ceramicParts', 'procure:supplies', 'conduct-experiment', 'procure:supplies', 'calibrate');
    assert.ok(!diagnoseRecord(g.session.record).findings.some(f => f.kind === 'product-unused'));
    assert.equal(g.o.productNetwork.goods.calibrations, 2);
    assert.equal(g.o.productNetwork.goods.spentCeramics, 2);
    assert.equal(g.o.development.goods.findings, 0);
    assert.equal(g.o.productNetwork.installed.calibrator, 2);
    ...
    while (g.o.world.weather !== 'dry' || g.o.ap < 3 || g.o.world.water < 2)
        await g.act('end-turn');
    const water = g.o.world.water, food = g.o.harvest.food;
    await g.act('pump-water');
    assert.equal(g.o.world.water, water - 1);
    assert.equal(g.o.productNetwork.storedWater, 1);
    await g.act('pump-water');
    assert.equal(g.o.world.water, water - 2);
    assert.equal(g.o.productNetwork.storedWater, 2);
    await assert.rejects(g.act('pump-water'));
    await g.act('end-turn');
    assert.equal(g.o.productNetwork.storedWater, 2);
    while (g.o.world.weather !== 'dry')
        await g.act('end-turn');
    assert.ok(g.o.harvest.food > food);
    assert.equal(g.o.harvest.storedDrawn, 2);
    await g.act('cultivate');
    assert.equal(g.o.productNetwork.storedWater, 0);
    ...
    await g.act('recycle-ceramics');
    assert.equal(g.o.productNetwork.goods.spentCeramics, 0);
    assert.equal(g.o.development.goods.ceramicParts, parts + 1);
    const frozen = JSON.stringify(g.session.record);
    await assert.rejects(g.act('recycle-ceramics'), /用后陶料/);
    assert.equal(JSON.stringify(g.session.record), frozen);
    await g.chain('gather:wood', 'gather:wood');
    const assets = structuredClone(g.o.productNetwork);
    while (g.o.status === 'active') {
        if (g.o.family.food < 3 && g.o.actions.find(a => a.id === 'gather:food')?.enabled)
            await g.act('gather:food');
        else
            await g.act('end-turn');
    }
    await g.act('handover');
    assert.deepEqual(g.o.productNetwork, { ...g.o.productNetwork, ...assets });
    assert.ok(!g.o.person.mastered.includes('mechanics'));
    await g.act('pump-water');
    assert.equal(g.o.productNetwork.installed.pump, assets.installed.pump - 1);
    const m = productNetworkMetrics(g.session.record);
    assert.equal(m.operations.calibrator, 2);
    assert.equal(m.operations.kiln, 1);
    assert.ok(m.usesByGeneration[2].includes('pump'));
