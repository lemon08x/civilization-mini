...
async function game(scenarioId = 'woodland', overrides = {}) {
...
test('储存损耗先扣生活再算余粮；容量候选只能放独立覆盖中', async () => {
    ...
    await unprotected.act('end-turn');
    await protectedGame.act('end-turn');
...
test('旧 v2 只能显式校验复制迁移，不能改成新世界或绕过指纹', async () => {
    ...
    const broken = structuredClone(old.record);
    broken.entries[0].stateHash = 'bad';
    await assert.rejects(importRecord(broken, implementation, 'broken'));
    const changed = structuredClone(old.record);
    changed.manifest.ruleset.parameters.workIncome++;
...
test('必要、替代和有利经验都校验依赖；机制不能伪装为旧规则', () => {
    ...
    rules.technologies.find(t => t.id === 'woodworking').helpfulPrerequisites = ['storage'];
    assert.throws(() => validateRuleset(rules), /成环/);
    const wrongVersion = structuredClone(base);
    wrongVersion.rulesVersion = '0.1.0';
