...
const fixture = JSON.parse(await readFile(join(projectRoot, 'tests/fixtures/legacy-v1.json'), 'utf8'));
...
test('命令幂等、过期拒绝、输入冻结与观察隔离', async () => {
    ...
    assert.equal(initial.state.ap, 3);
    ...
    assert.ok(!observation.includes('randomState'));
    assert.ok(!observation.includes('codeFingerprint'));
});
test('篡改轨迹、配置、快照和实现指纹均拒绝', async () => {
    ...
    for (const mutate of [
        (s) => { s.entries[0].stateHash = 'broken'; },
        (s) => { s.manifest.ruleset.parameters.initialFood++; },
        (s) => { s.snapshots[0].state.ap++; },
        (s) => { s.manifest.implementation.version = '9.0.0'; },
    ]) {
        const copy = structuredClone(session.record);
        mutate(copy);
        await assert.rejects(replayRecord(copy, implementation));
    }
...
test('文件保存保留历史，拒绝覆盖、重复提交和路径穿越', async () => {
    ...
    await store.create(await fresh());
    await assert.rejects(store.create(await fresh()));
    ...
    for (const layer of ['game', 'runtime']) {
        ...
        for (const file of (await readdir(root, { recursive: true })).filter(f => f.endsWith('.ts'))) {
            ...
            for (const match of source.matchAll(/from\s+['"]([^'"]+)['"]/g)) {
                ...
                if (layer === 'game')
                    assert.ok(!/runtime|node:/.test(match[1]));
            }
        }
    }
});
