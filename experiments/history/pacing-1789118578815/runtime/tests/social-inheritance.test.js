...
async function game(overrides = {}, scenarioId = 'woodland') {
    ...
    return { get session() { return session; }, get o() { return observeSession(session).game; },
        async act(...ids) { for (const actionId of ids)
            session = (await submitCommand(session, { commandId: `s:${session.record.entries.length}`, expectedRevision: session.record.entries.length, actionId })).session; },
    };
}
async function wood(g) {
    await g.act('study:woodworking', 'study:woodworking', 'gather:wood', 'craft:woodenware', 'finish-craft');
}
async function business(g) {
    await wood(g);
...
test('公共商品可购买并扣钱扣库存，不赠送制造技能；无资金/原料不扣费', async () => {
    const g = await game({ initialMoney: 1 });
    await business(g);
    await g.act('entrust:woodenware', 'end-turn');
...
test('社会观察不泄露随机状态，旧规则不注入社会且历史指纹不猜测迁移', async () => {
    ...
    const historical = structuredClone(old.record);
    historical.manifest.implementation.version = '0.4.0';
...
test('受托匠人与玩家共用当地原料，材料耗尽时不扣开工工资', async () => {
    const g = await game({ actionsPerTurn: 6 });
    await business(g);
    await g.act('entrust:woodenware');
    ...
    if (!g.session.record.entries.slice(start).flatMap(e => e.events).some(e => e.type === 'season-settled'))
        await g.act('end-turn');
    assert.equal(g.o.family.money, money);
    assert.equal(g.o.society.contracts.woodenware.project, null);
...
test('陶作委托保留跨季工序，受一单限制时分次交货且不重复收工资', async () => {
    ...
    const gatherTo = async (resource, amount) => {
    ...
    await gatherTo('wood', 5);
    await gatherTo('clay', 5);
    if (g.o.ap < 2)
        await g.act('end-turn');
