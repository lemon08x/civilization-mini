...
async function game(scenarioId = 'woodland') {
    ...
    return { get session() { return session; }, get o() { return observeSession(session).game; }, async act(...ids) { for (const actionId of ids)
...
test('新规则与框架独立版本，参数有界，旧规则没有新增状态或行动', async () => {
    ...
    assert.equal(f.nodes.length, 34);
    assert.equal(f.nodes.filter(n => n.status === 'implemented').length, 17);
...
test('完成制作才长技能，工业扣料一次、共用项目位，失败不改变记录', async () => {
    ...
    while (g.o.production.inventory.wood < 3)
    ...
    assert.equal(g.o.development.goods.ceramicParts, 0);
    const before = JSON.stringify(g.session.record);
    await assert.rejects(g.act('craft:woodenware'), /项目/);
    await assert.rejects(g.act('develop:mechanisms'));
    assert.equal(JSON.stringify(g.session.record), before);
    await g.act('finish-development');
    assert.equal(g.o.development.goods.mechanisms, 1);
    assert.equal(g.o.production.inventory.wood, wood - 3);
...
test('农业补给实际进入科学实验、工艺改良，实习与实物跨代保留', async () => {
    ...
    assert.equal(g.o.development.goods.supplies, 1);
    assert.equal(g.o.development.goods.findings, 1);
    await g.act('refine:agriculture', 'mentor:agriculture');
    assert.equal(g.o.development.goods.findings, 0);
    assert.equal(g.o.development.goods.supplies, 0);
    ...
    while (g.o.status === 'active') {
        if (g.o.family.food < 4)
            await g.act('gather:food');
        else
            await g.act('end-turn');
    }
    await g.act('handover');
    assert.deepEqual(g.o.development.designs, assets.designs);
    assert.deepEqual(g.o.development.project, assets.project);
    ...
    await g.act('finish-development');
    assert.equal(g.o.development.goods.supplies, 2);
    assert.equal(developmentMetrics(g.session.record).refinements, 1);
    assert.deepEqual((await replayRecord(g.session.record, implementation)).state, g.session.state);
});
test('采购不加经验，买卖不套利，设备不叠加、真实使用才损耗', async () => {
    const g = await game();
    await g.act('procure:fieldTools', 'equip:field');
    const m = g.o.family.money;
    await g.act('procure:supplies', 'deliver:supplies');
    assert.equal(g.o.family.money, m - 1);
    assert.ok(g.o.development.skills.every(s => s.experience === 0));
    await assert.rejects(g.act('equip:field'));
    await g.act('cultivate');
    const harvest = g.session.record.entries.at(-1).events.find(e => e.type === 'harvest');
    assert.ok(harvest);
...
test('陶作跨季后才完工，构件继续积累陶作经验', async () => {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        if (!g.o.actions.find(a => a.id === 'finish-craft').enabled)
            await g.act('end-turn');
        await g.act('finish-craft');
    }
    await g.act('study:ceramic-engineering');
    while (g.o.production.inventory.wood < 2) {
        if (g.o.production.stocks.timber === 0)
            await g.act('end-turn');
        else
            await g.act('gather:wood');
    }
    while (g.o.production.inventory.clay < 2)
        await g.act('gather:clay');
    if (g.o.ap === 1)
        await g.act('end-turn');
...
test('实验设备提高研究产出；持续陶作可进入精密制造并交付', async () => {
    ...
    async function start(id) {
        for (let i = 0; i < 30; i++) {
            ...
            const needed = Object.entries(a.materials ?? {}).find(([m, n]) => g.o.production.inventory[m] < n);
            if (needed && g.o.actions.find(a => a.id === `gather:${needed[0]}`).enabled)
                await g.act(`gather:${needed[0]}`);
            else if (g.o.family.food < 4 && g.o.actions.find(a => a.id === 'cultivate').enabled)
                await g.act('cultivate');
            else
                await g.act('end-turn');
        }
    ...
    async function finish(id) { if (!g.o.actions.find(a => a.id === id).enabled)
        await g.act('end-turn'); await g.act(id); }
    for (let i = 0; i < 2; i++) {
        await start('craft:pottery');
        await finish('finish-craft');
    }
    await g.act('study:ceramic-engineering');
    for (let i = 0; i < 3; i++) {
        await start('develop:ceramicParts');
        await finish('finish-development');
    }
    ...
    if (!g.o.actions.find(a => a.id === 'procure:supplies').enabled)
        await g.act('end-turn');
    await g.act('procure:supplies');
    await start('develop:precisionParts');
    const parts = g.o.development.goods.ceramicParts;
    await finish('finish-development');
    assert.equal(g.o.development.goods.precisionParts, 1);
    assert.equal(g.o.development.goods.ceramicParts, parts);
    await g.act('deliver:precisionParts');
    assert.equal(developmentMetrics(g.session.record).deliveryIncome, 17);
    await g.act('study:experimentation', 'procure:labTools', 'equip:lab');
    if (!g.o.actions.find(a => a.id === 'procure:supplies').enabled)
        await g.act('end-turn');
    await g.act('procure:supplies');
    if (g.o.family.food < 3)
        await g.act('buy-food');
    await g.act('conduct-experiment');
    assert.equal(g.o.development.goods.findings, 2);
    assert.equal(g.o.development.labDurability, 3);
    assert.deepEqual((await replayRecord(g.session.record, implementation)).state, g.session.state);
});
