...
export async function loadResearchIndex(root: string, rules?: Ruleset) {
  ...
  if (rules) try { framework = validateFramework(JSON.parse(await readReviewFile(root,rules.productNetwork?'experiments/frameworks/knowledge.v3.json':rules.development?'experiments/frameworks/knowledge.v2.json':'experiments/frameworks/knowledge.v1.json')),rules); } catch(e) { issues.push(`知识框架：${(e as Error).message}`); }
