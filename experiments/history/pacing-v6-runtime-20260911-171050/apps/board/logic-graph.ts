...
export function buildLogicGraph(r:Ruleset) {
  ...
  if(r.production) {
    ...
    process('craft:wood','制作木制品','woodworking',`开工消耗${p.woodRecipeCost}木材，再用行动完成；木容器与采集工具是不同配方。完成积累木作经验。`);
    ...
    process('craft:pottery','成形与烧陶','pottery',`投入${p.potteryClayCost}黏土、${p.potteryFuelCost}木材；跨季干燥后完成。积累陶作经验。`);
