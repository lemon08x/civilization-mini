import { PASSIVE_EFFECTS } from '../../src/game/systems/investment.js';
...
export function buildLogicGraph(r:Ruleset) {
  ...
  if(r.productNetwork) {
    for(const recipe of DEVICE_RECIPES) {
      const id='fabricate:'+recipe.id;process(id,'制造'+PRODUCT_NAMES[recipe.id],recipe.method,'开工、完成各1行动，与其他在制品共用项目槽。'+(r.passiveInvestment?PASSIVE_EFFECTS[recipe.id]:recipe.effect));
    ...
    process('calibrate','安装后进行校准','experimentation',r.passiveInvestment?'安装即启用，季末自动校准，每季至多一次，不花行动；1记录+1补给+1耐用度→1报告。自动生产不增加个人经验。': '先安装校准仪；每次校准耗1研究记录、1补给、1耐用度，取得1报告和1科学经验。');
    ...
    process('pump-water','安装后提水','',(r.passiveInvestment?'安装时及季末自动运行，每季至多一次，不花行动。':'')+'消耗1木材、1设备耐用，将1公共水存入家庭；容量2，无公共水不能提水。');good('storedWater','家用蓄水','跨季保留；缺水耕作先用蓄水，容量2。');good('publicWater','公共水','受世界过程影响的有限水源；提水会扣除公共水。');link('g:pump','pump-water','安装后开启');link('g:wood','pump-water','消耗1','input');link('g:publicWater','pump-water','转移1','input');link('pump-water','g:storedWater','存入1','output');link('g:storedWater','harvest','缺水耕作消耗');
    process('recycle','安装后回收烧结','',(r.passiveInvestment?'安装时及季末自动运行，每季至多一次，不花行动，不增个人经验。':'获得1陶作经验。')+'2用后陶料 + 1木材 + 1耐用度 → 1陶质构件。回收有损耗。');link('experiment','g:spentCeramics','留下1','output');link('g:kiln','recycle','安装后开启');link('g:spentCeramics','recycle','消耗2','input');link('g:wood','recycle','消耗1','input');link('recycle','g:ceramicParts','回收1','output');
  }
  if(r.passiveInvestment){process('sell-calibration','出售校准报告','',`消耗1报告及1专业订单，收入${r.passiveInvestment.reportPrice}钱。`);link('g:calibrations','sell-calibration','交付1','input');}
