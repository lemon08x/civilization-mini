...
export function productCatalog(r:Ruleset){
 ...
 const nodes:ProductNode[]=graph.nodes.filter(n=>n.kind===2).map(n=>({id:n.id,name:n.name,category:groups[n.id.slice(2)]?.[0]??'材料与金属',stage:groups[n.id.slice(2)]?.[1]??0,planned:false,detail:n.id==='g:findings'?'由真实材料实验产生，可消耗于工艺改良或仪器校准。不是通用科技点。':n.id==='g:calibrations'?(r.passiveInvestment?'安装校准仪后自动产出，可出售或用于提水泵和控温窑制造。':'由校准操作产生，用于提水泵和控温窑制造。不是自动解锁全部科技。'):n.detail}));
