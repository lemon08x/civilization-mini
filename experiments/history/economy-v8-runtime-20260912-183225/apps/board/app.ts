...
const KEY = 'civilization-mini.rules-lab.v9';
...
const ruleset = validateRuleset(await (await fetch('/rulesets/passive-investment.v8.json')).json());
...
function render() {
  ...
  $('app').innerHTML = `
    ...
    ${end ? `<section class="terminal"><h2>${o.victory?.won ? '科技胜利' : o.status === 'complete' ? '观察期结束，未达科技胜利' : '实验因连续生活缺口终止'}</h2><p>累计耕作产粮 ${results.foodProduced}${results.production ? `、采食 ${results.production.gatheredFood}、损耗 ${results.production.spoiledFood}、手工交换收入 ${results.production.craftEarnings}` : ''}，生活缺口 ${results.foodShortfall}，用水 ${results.waterUsed}。掌握过 ${results.learnedNodes.length} 个节点，取得 ${results.samples} 次试种记录。</p><p>科技胜利以家族历代去重掌握比例判定；生活、设施与传承仍分别记录。可导出轨迹回顾具体取舍。</p></section>` : ''}
    ${progressPanel(o)}
    ${lifePanel(o)}
...
function productNetworkPanel(o:BoardView):string {
  ...
  return `<section class="panel"><div class="panel-head"><h2>高级设备与收益</h2><small>库存 · 安装 · 使用</small></div><div class="panel-body">
    ...
    ${n.project?offer('finish-product'):''}${offer('sell-calibration')}${offer('calibrate')}${offer('pump-water')}${offer('recycle-ceramics')}
  </div></section>`;
}
...
function developmentPanel(o:BoardView):string {
  ...
  return `<section class="panel development"><div class="panel-head"><h2>手艺成长、工业与科学</h2><small>真实实践积累 · 设备与工艺留给家族</small></div><div class="panel-body">
    ...
    <div class="family-grid">${d.skills.map(s=>`<div class="family-item"><strong>${DISCIPLINE_NAMES[s.domain]} · 等级 ${s.level}</strong><p>经验 ${s.experience} / 下一等级 ${s.next} · 后辈 ${s.heirExperience}<br>家族工艺改良 ${d.designs[s.domain]} 次</p><p>当前可传授上限 ${Math.floor(s.experience/2)} 经验</p>${offer(`mentor:${s.domain}`)}<progress max="${s.next}" value="${s.experience}" aria-label="${DISCIPLINE_NAMES[s.domain]}经验"></progress></div>`).join('')}</div>
    ...
    <details><summary>出售工业品并补充口粮</summary>${o.actions.filter(a=>a.id.startsWith('deliver-food:')).map(a=>offer(a.id)).join('')}</details>
