...
运行产物只写 artifacts/。旧 --file 存档请显式 import，不原地覆盖。`);
    ...
    if (opts.rules && !['legacy', 'production', 'feedback', 'society', 'development', 'network', 'pacing', 'investment'].includes(opts.rules)) throw new Error('--rules 只支持 legacy / production / feedback / society / development / network / pacing / investment');
    const { implementation, legacyBase } = context, base = opts.rules === 'legacy' ? legacyBase : opts.rules === 'production' ? context.productionBase : opts.rules === 'feedback' ? context.feedbackBase : opts.rules === 'society' ? context.societyBase : opts.rules === 'development' ? context.developmentBase : opts.rules === 'network' ? context.base : opts.rules === 'pacing' ? context.pacingBase : context.investmentBase;
