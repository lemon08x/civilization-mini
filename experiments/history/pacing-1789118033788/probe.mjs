...
function controller(mode){let phase=0;return observation=>{
 ...
 const eat=()=>{const options=[['cultivate',g.harvest.food],['gather:food',g.production.gathering.food],['buy-food',g.rulesVersion==='0.7.0'&&(g.production.storage.woodenware||g.production.storage.pottery)?4:2]].filter(([id,n])=>enabled(id)&&n>0).sort((a,b)=>b[1]-a[1]);return options[0]?.[0]??(enabled('work')?'work':'end-turn');};
 ...
 if(g.clock.generation<3&&g.clock.turn>=13){for(const id of ['woodworking','controlled-fire','pottery','ceramic-engineering','experimentation'])if(has(id)&&!g.heir.mastered.includes(id)){if(!g.family.archives.includes(id))return pay('archive:'+id);if(enabled('teach:'+id))return pay('teach:'+id);}}
 if(mode==='water')return !g.family.channel?(method('ditch')||pay('build-channel')):g.family.channel.durability===0?pay('repair-channel'):!has('allocation')?master('allocation'):enabled('cultivate')&&g.harvest.food>0?pay('cultivate'):enabled('work')?'work':'end-turn';
 const goals=[()=>install('calibrator'),()=>g.productNetwork.goods.calibrations>0?null:calibration(),()=>install('pump'),()=>g.productNetwork.storedWater>0?null:pay('pump-water'),()=>install('kiln'),()=>g.productNetwork.goods.spentCeramics>=2?null:(method('experimentation')||need('supplies')||need('ceramicParts')||pay('conduct-experiment')),()=>pay('recycle-ceramics')];
...
for(const mode of ['self','purchase','water'])for(const [variant,ruleset] of [['v6',base],['candidate',candidate]]){
