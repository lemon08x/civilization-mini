...
function actionLabel(action: GameAction, rules: Ruleset): string {
  ...
  if (action.type === 'procure' || action.type === 'deliver') return (action.type === 'procure' ? '外购' : '交付') + GOOD_NAMES[action.good];
...
export function formatEvents(events: GameEvent[], rules: Ruleset): string[] {
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case 'product-completed': text.push(NETWORK_NAMES[event.device]+'装配完成；安装后开启专用操作。'); break;
      case 'product-installed': text.push(NETWORK_NAMES[event.device]+'已安装，可用'+event.durability+'次。'); break;
      case 'product-operated': text.push(NETWORK_NAMES[event.device]+'操作完成，'+({calibrator:'消耗1记录和1补给，产出1校准报告',pump:'公共水减少1，家用蓄水增加1',kiln:'消耗2用后陶料，重制1陶质构件'}[event.device])+'；设备剩余'+event.remaining+'次。'); break;
