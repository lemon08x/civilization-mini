...
运行产物只写 artifacts/。旧 --file 存档请显式 import，不原地覆盖。`);
    ...
    if (opts.rules && !['legacy', 'production', 'feedback', 'society', 'development', 'network', 'pacing'].includes(opts.rules)) throw new Error('--rules 只支持 legacy / production / feedback / society / development / network / pacing');
