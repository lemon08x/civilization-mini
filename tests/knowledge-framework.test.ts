...
test('跨领域框架覆盖现行节点，拒绝伪装实现、缺失关系和解锁环', async()=>{
  ...
  for(const mutate of [
    ...
    (v:typeof f)=>{v.nodes.find(n=>n.id==='steam')!.required=['missing'];},
    (v:typeof f)=>{v.nodes.find(n=>n.id==='metallurgy')!.required=['steam'];},
    (v:typeof f)=>{v.nodes.find(n=>n.id==='woodworking')!.required=['steam'];},
    (v:typeof f)=>{v.questions[0].parameter='socialInheritance.wage';},
  ]) {const copy=structuredClone(f);mutate(copy);assert.throws(()=>validateFramework(copy,base));}
  // 促进与讨论可相互影响，不当成必须先学的循环。
  f.nodes.find(n=>n.id==='logic')!.helpful=['ethics']; f.nodes.find(n=>n.id==='ethics')!.helpful=['logic'];
  assert.doesNotThrow(()=>validateFramework(f,base));
});

function record(events: GameEvent[]): RunRecord {
  return {manifest:{ruleset:{technologies:[{id:'woodworking'},{id:'pottery'}]}},entries:events.map((event,i)=>({revision:i+1,events:[event]}))} as RunRecord;
}
test('诊断按人物区分学习，不把继承人掌握误作当前人物完成；证据定位原事件',()=>{
  ...
  r.entries.push({revision:3,events:[{type:'mastered',personId:'parent',role:'active',nodeId:'woodworking',clock:{generation:1,turn:2,absoluteTurn:2}}]} as RunRecord['entries'][number]);
  assert.equal(diagnoseRecord(r).findings.some(f=>f.kind==='learning-unfinished'),false);
});
test('设施诊断只识别建成之后同类批量完工，不把普通产出或其他工艺混入',()=>{
  const events:GameEvent[]=[{type:'craft-completed',recipe:'woodenware',amount:2,batch:true},{type:'workshop-built',material:'woodenware'},{type:'craft-completed',recipe:'woodenware',amount:1},{type:'craft-completed',recipe:'pottery',amount:4,batch:true}];
