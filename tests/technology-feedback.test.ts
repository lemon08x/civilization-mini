...
async function game(scenarioId = 'woodland') {
  ...
  return {
    ...
    async act(...ids: string[]) {
      for (const actionId of ids) session = (await submitCommand(session, { commandId: `test:${session.record.entries.length}`, expectedRevision: session.record.entries.length, actionId })).session;
    },
  };
}
async function woodworker(g: Awaited<ReturnType<typeof game>>) {
  await g.act('study:woodworking', 'study:woodworking', 'gather:wood', 'craft:woodenware', 'finish-craft');
}
async function gatherTo(g: Awaited<ReturnType<typeof game>>, resource: 'wood' | 'clay', target: number) {
...
test('陶窑批量消耗双份燃料和黏土，跨季后产四件，失败不扣材料', async () => {
  ...
  await gatherTo(g, 'wood', 4); await gatherTo(g, 'clay', 4);
...
test('设施和批量在制品跨代保留，未受教学的后辈只能接续不能新开批量', async () => {
  ...
  while (g.o.status === 'active') await g.act('cultivate', 'end-turn');
