...
const check = (directory: string) => execute(process.execPath,[script,directory],{cwd:projectRoot});
...
test('研究校验遇到混合指纹立即非零退出，不生成部分成功报告、不改原档',async()=>{
  ...
  await assert.rejects(check(directory),(error: Error & {code?:number;stderr?:string})=>error.code!==0 && /02-invalid.json: implementation fingerprint mismatch/.test(error.stderr??''));
...
test('研究校验全通过才保存成功结果，已有报告拒绝覆盖',async()=>{
  ...
  assert.equal(summary.verified,1);assert.equal(summary.failed,0);
