...
import type {RunRecord} from '../src/runtime/records.js';
...
function record(...groups:GameEvent[][]):RunRecord {return {manifest:{ruleset:base},entries:groups.map((events,i)=>({revision:i+1,events}))} as RunRecord;}
const season=(turn:number,weather:'dry'|'normal'='dry'):GameEvent=>({type:'season-started',clock:{generation:1,turn,absoluteTurn:turn},weather});
const harvest:GameEvent={type:'harvest',food:2,drawn:0,deficit:1,channelExhausted:false};
const water:GameEvent={type:'stored-water-used',amount:1};
test('经历区分制成、安装与实际使用，不把具备条件写成收益',()=>{
  ...
  assert.equal(JSON.stringify(r),before);assert.ok(!('score' in c));
...
test('跨代设备使用追溯最后一次安装，不冒认更换后的设备为前代遗产',()=>{
  const install:GameEvent={type:'product-installed',device:'pump',durability:4},use:GameEvent={type:'product-operated',device:'pump',result:1,remaining:3};
  const handover:GameEvent={type:'handed-over',generation:2,fromPersonId:'person:1',personId:'person:2',mastered:[],learning:{}};
...
test('后辈、邻里与受托匠人的事实不归功于当前玩家；重复经历不变为刷分',()=>{
  const e:GameEvent={type:'public-used',material:'pottery',amount:1};
  ...
  assert.equal(c.chapters[0].facts.length,2);assert.equal(c.chapters[0].facts[0].actor,'邻里');
  assert.equal(c.chapters[0].facts[1].actor,'后辈');assert.equal(c.chapters[0].facts[1].stage,'prepared');
});
test('真实行动生成经历不改变结算、观察、记录或存档指纹',async()=>{
  ...
  for(const actionId of ['study:woodworking','gather:wood','craft:woodenware','finish-craft'])s=(await submitCommand(s,{commandId:`life:${s.record.entries.length}`,expectedRevision:s.record.entries.length,actionId})).session;
  const raw=JSON.stringify(s.record),o=JSON.stringify(observeSession(s));
  const c=lifeChronicle(s.record);assert.ok(c.chapters[0].facts.some(f=>f.id==='craft:woodenware'));assert.equal(c.chapters[0].investment.actions,4);
  assert.equal(JSON.stringify(s.record),raw);assert.equal(JSON.stringify(observeSession(s)),o);
