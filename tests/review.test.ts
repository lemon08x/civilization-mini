...
test('研究文件读取拒绝路径穿越和游戏存档目录，不覆盖原文件', async () => {
  ...
  for (const path of ['../package.json', 'docs/../package.json', 'docs/../../secrets.json', 'artifacts/runs/demo/record.json', 'apps/board/server.ts', 'docs\\..\\package.json']) await assert.rejects(readReviewFile(projectRoot, path));
