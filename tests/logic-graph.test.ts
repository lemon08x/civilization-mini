...
test('现行逻辑图覆盖全部科技和生产配方，关系无悬空，技能与材料门槛不混淆',()=>{
 ...
 for(const t of base.technologies)assert.ok(ids.has(t.id));
 ...
 for(const g of Object.keys(r.inputs))assert.ok(edges.some(e=>e.from==='g:'+g&&e.to===id&&e.kind==='input'));
 }
...
test('科技主图只包含现行科技，材料与技能条件留在节点详情',()=>{
 ...
 for(const id of targets)assert.ok(techIds.has(id));
 for(const id of [...html.matchAll(/data-tech="([^"]+)"/g)].map(m=>m[1]))assert.ok(techIds.has(id));
